...
StylesAdded
Printout
Function GetReceiptDetails(drgsaleid)
    Dim rst, obj
    ...
    obj.Add "date", ""
    sql = ""
    sql = sql & "SELECT Staff.StaffName,Receipt.ReceiptDate "
    sql = sql & "FROM Receipt "
    sql = sql & "INNER JOIN DrugSale ON DrugSale.MainInfo2 = Receipt.ReceiptID "
    sql = sql & "INNER JOIN SystemUser ON SystemUser.Systemuserid = Receipt.Systemuserid "
    sql = sql & "INNER JOIN Staff ON Staff.StaffID = SystemUser.StaffID "
    sql = sql & "WHERE DrugSale.DrugSaleID = '" & drgsaleid & "' "
    rst.open sql, conn, 3, 4
    If rst.recordCount > 0 Then
        obj("cashier") = rst.fields("StaffName")
        obj("date") = FormatDateTime(rst.fields("ReceiptDate"))
    End If
    rst.Close
    Set rst = Nothing
    GetReceiptDetails = obj
End Function
Sub Printout()

    Dim receiptD
    Set receiptD = GetReceiptDetails(drugsaleid)
    'Dim bal
    ...
    response.write "         <div class='header-style' style='width:100;'>"
    response.write "            <span>Date: " & receiptD("date") & "<span>"
    response.write "         </div>"
    response.write "         <div class='header-style' style='width:100;display:flex;justify-content:space-between'>"
    response.write "         <label>ItemSaleID: " & drugsaleid & "</label>"
    response.write "         <label>Cashier: " & receiptD("cashier") & "</label>"
    response.write "         </div>"
    response.write "         <tr>"
