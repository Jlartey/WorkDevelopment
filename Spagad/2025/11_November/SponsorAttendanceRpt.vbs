...
Sub generateReport()
    Dim rst, sql, cnt, SponsorID, datePeriod, specialistTypeID, ageGroupID, VisitationID
...
'    sql = sql & " GROUP BY Visitation.insuranceNo, visitation.patientID, Patient.ResidencePhone, visitation.InsSchemeModeID, visitation.VisitTypeID, visitation.GenderID"
    ...
    sql = sql & " Visitation.insuranceNo AS MembershipID,"
    sql = sql & " Visitation.VisitDate AS VisitDate,"
    sql = sql & " visitation.patientID,"
    sql = sql & " visitation.InsSchemeModeID,"
    sql = sql & " visitation.VisitTypeID,"
    sql = sql & " visitation.GenderID,"
    sql = sql & " Patient.ResidencePhone,"
    sql = sql & " Sponsor.SponsorName,"
    sql = sql & " SpecialistType.SpecialistTypeName,"
    'sql = sql & " InitialDependant.InitialDependantName"
    sql = sql & " CASE WHEN InitialDependant.InitialDependantName = 'NONE' THEN"
    sql = sql & " 'MAIN HOLDER' ELSE InitialDependant.InitialDependantName END"
    sql = sql & " AS InitialDependantName"
    sql = sql & " From visitation"
    sql = sql & " INNER JOIN Patient ON visitation.patientID = Patient.PatientID"
    sql = sql & " Join InsuredPatient"
    sql = sql & " ON InsuredPatient.PatientID = Patient.PatientID"
    sql = sql & " Join sponsor"
    sql = sql & " ON Visitation.SponsorID = Sponsor.SponsorID"
    sql = sql & " Join specialistType"
    sql = sql & " ON SpecialistType.SpecialistTypeID = Visitation.SpecialistTypeID"
    sql = sql & " Join InitialDependant"
    sql = sql & " ON InitialDependant.InitialDependantID = Visitation.InitialDependantID"
    sql = sql & " WHERE Visitation.VisitDate BETWEEN '" & periodStart & "'  AND  '" & periodEnd & "'"
    
    If SponsorID <> "" Then
        sql = sql & " AND Visitation.sponsorID = '" & SponsorID & "'"
    Else
        sql = sql & " "
    End If
    ...
    Else
        sql = sql & " "
    End If
    'sql = sql & " AND Patient.PatientID = 'IMAH-25-005188'"
    ...
    If .recordCount > 0 Then
        ...
        response.write "            <td colspan='13'><b>ATTENDANCE REPORT FOR " & GetComboName("Sponsor", SponsorID) & " IN " & GetComboName("WorkingYear", workingYearID) & "</b></td>"
        response.write "        </tr> "
        ...
        Do While Not .EOF
            ...
            response.write "            <td>" & VisitationID & "</td>"
            ...
            response.write "            <td>" & .fields("SpecialistTypeName") & "</td>"
            response.write "            <td>" & .fields("InitialDependantName") & "</td>"
            response.write "            <td>" & GetConsultingDoctor(VisitationID) & "</td>"
