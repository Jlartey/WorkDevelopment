...
Dim sql, cnt
Dim arPeriod, Gender, MaritalStatus, country, dateRange, visitType, ageGroup
...
visitType = Trim(Request.QueryString("PrintFilter4"))
ageGroup = Trim(Request.QueryString("PrintFilter5"))
...
Sub kyc
    ...
    If Len(visitType) > 0 Then
        sql = sql & " AND Visitation.VisitTypeID = '" & visitType & "' "
    Else
        sql = sql & " "
    End if

     If Len(ageGroup) > 0 Then
    ...
    If IsArray(dateRange) And UBound(dateRange) >= 1 And Len(Trim(dateRange(0))) > 0 And Len(Trim(dateRange(1))) > 0 Then
        ...
        sql = sql & " AND NOT EXISTS (SELECT 1 FROM Visitation AS V2 WHERE V2.PatientID = Patient.PatientID AND V2.VisitDate < '" & formattedDates(0) & "') "
    End If
    ...
    With rst0
        ...
        If .recordCount > 0 Then
            ...
            response.write "<b>" & uniqueCount & " first-time unique patients</b></td></tr>"
