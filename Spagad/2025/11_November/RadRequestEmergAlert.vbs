...
Sub SetupAlert()
  ...
  sql = "select count(distinct VisitationID) as cnt from LabByDoctor "
  sql = sql & " where BranchID='" & currBr & "' and LabByDoctorAmt2=1 and TestGroupID='B19' and PrescriptionDate between '" & FormatDateDetail(sDt) & "' and '" & FormatDateDetail(currDt) & "'"
  With rst
    .open sql, conn, 3, 4
  ...
  End With
  'If noPat > 0 Then
    ot = "<b><u><font color=""red"">Emergency Radiology Request [Past 24 hours]</font></u></b><br>"
    
    'Clickable Url Link
    lnkUrl = "wpgPrtPrintLayoutAll.asp?PrintLayoutName=MonitorVisitationRadEmerg&PositionForTableName=WorkingDay&WorkingDayID=DAY20160401"
    ...
    sql = "select LabByDoctorStatusID,count(LabTestID) as cnt from LabByDoctor"
    sql = sql & " where BranchID='" & currBr & "' and LabByDoctorAmt2=1 and TestGroupID='B19' and PrescriptionDate between '" & FormatDateDetail(sDt) & "' and '" & FormatDateDetail(currDt) & "'"
        ...
        Do While Not .EOF
          If Not IsNull(.fields("cnt")) Then
            If IsNumeric(.fields("cnt")) Then
              ...
              cnt = .fields("cnt")
              ot = ot & "<br><b>" & GetComboName("LabByDoctorStatus", ky) & " :</b>&nbsp;&nbsp;" & CStr(cnt)
            End If
          End If
          .MoveNext
        Loop
