...
'Function DrugsSold01(visitationID)
'    Dim sql, rst, htmlOutput, drugString, drugArray, i
'
'    Set rst = CreateObject("ADODB.Recordset")
'
'    sql = "WITH CombinedDrugSales AS ( " & _
...
'          "    WHERE VisitationID = '" & visitationID & "' " & _
'          ") " & _
'          "SELECT VisitationID, " & _
'          "       STRING_AGG(CAST(d.DrugName AS VARCHAR(MAX)), ', ') AS [Drugs Dispensed] " & _
'          "FROM CombinedDrugSales cds " & _
'          "JOIN Drug d " & _
'          "    ON cds.DrugID = d.DrugID " & _
...
'
'    If Not rst.EOF Then
'        drugString = rst("Drugs Dispensed")
