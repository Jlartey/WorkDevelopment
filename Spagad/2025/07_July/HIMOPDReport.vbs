...
Function ConsultationReportSQL(dateRange, ageGroup, spType)
    Dim sql
    sql = "SELECT DISTINCT sp.SpecialistTypeName AS [Consultation Type],"
    ...
    sql = sql & "dg.DiagnosisType AS [Diagnosis Type],"
    sql = sql & "temp.temperature AS [Temperature],"
    sql = sql & "wg.weight12 AS [Weight],"
    sql = sql & "ht.height AS [Height],"
    'sql = sql & "bp.BloodPressure AS [BP],"
    ...
    sql = sql & "LEFT JOIN Diagnosis AS dg ON dg.VisitationID=vst.VisitationID "
    
    
    sql = sql & " LEFT JOIN ("
    sql = sql & " SELECT DISTINCT visitationid, CONVERT(NVARCHAR(MAX), Column3) AS Temperature FROM EMRResults AS emrres"
    sql = sql & " JOIN EMRRequest emrreq ON emrres.EMRRequestID = emrreq.EMRRequestID"
    sql = sql & " WHERE emrdataid ='EMR034' AND EMRComponentID='EMR03410') AS temp ON temp.visitationid=vst.VisitationID"
    sql = sql & " LEFT JOIN ("
    sql = sql & " SELECT DISTINCT visitationid, CONVERT(NVARCHAR(MAX), Column2) AS Weight12 FROM EMRResults AS emrres"
    sql = sql & " JOIN EMRRequest emrreq ON emrres.EMRRequestID = emrreq.EMRRequestID"
    sql = sql & " WHERE emrdataid ='EMR034' AND EMRComponentID='EMR05003') AS wg ON wg.visitationid=vst.VisitationID"
    sql = sql & " LEFT JOIN ("
    sql = sql & " SELECT DISTINCT visitationid, CONVERT(NVARCHAR(MAX), Column4) AS height FROM EMRResults AS emrres"
    sql = sql & " JOIN EMRRequest emrreq ON emrres.EMRRequestID = emrreq.EMRRequestID"
    sql = sql & " WHERE emrdataid ='EMR034' AND EMRComponentID='EMR05004') AS ht ON ht.visitationid=vst.VisitationID"
    
    'sql = sql & "LEFT JOIN (SELECT emrreq.visitationid,COLUMN2 AS temperature,EMRDate1 "
    'sql = sql & "FROM EMRResults AS emrres JOIN EMRRequest emrreq "
    'sql = sql & "ON emrres.EMRRequestID = emrreq.EMRRequestID "
    'sql = sql & "WHERE emrdataid = 'EMR050'  AND emrcomponentid = 'EMR05001' "
    'sql = sql & "AND EMRDate1 = (SELECT MIN(EMRDate1) FROM "
    'sql = sql & "EMRRequest WHERE emrdataid = 'EMR050'  AND emrcomponentid = 'EMR05001' "
    'sql = sql & "AND emrreq.visitationid = visitationid)) AS temp "
    'sql = sql & "ON temp.visitationid=vst.VisitationID "
    'sql = sql & "LEFT JOIN (SELECT emrreq.visitationid,COLUMN2 AS WEIGHT12,EMRDate1 "
    'sql = sql & "FROM EMRResults AS emrres JOIN EMRRequest emrreq "
    'sql = sql & "ON emrres.EMRRequestID = emrreq.EMRRequestID "
    'sql = sql & "WHERE emrdataid = 'EMR050'  AND emrcomponentid = 'EMR05003' "
    'sql = sql & "AND EMRDate1 = (SELECT MIN(EMRDate1) FROM "
    'sql = sql & "EMRRequest WHERE emrdataid = 'EMR050'  AND emrcomponentid = 'EMR05003' "
    'sql = sql & "AND emrreq.visitationid = visitationid)) AS wg "
    '    sql = sql & "ON wg.visitationid = vst.VisitationID "
    '    sql = sql & "LEFT JOIN (SELECT emrreq.visitationid,COLUMN4 AS height,EMRDate1 "
    '    sql = sql & "FROM EMRResults AS emrres JOIN EMRRequest emrreq "
    '    sql = sql & "ON emrres.EMRRequestID = emrreq.EMRRequestID "
    '    sql = sql & "WHERE emrdataid = 'EMR050'  AND emrcomponentid = 'EMR05003' "
    '    sql = sql & "AND EMRDate1 = (SELECT MIN(EMRDate1) FROM "
    '    sql = sql & "EMRRequest WHERE emrdataid = 'EMR050'  AND emrcomponentid = 'EMR05003' "
    '    sql = sql & "AND emrreq.visitationid = visitationid)) AS ht "
    '    sql = sql & "ON ht.visitationid = vst.VisitationID "
    '    sql = sql & "LEFT JOIN (SELECT emrreq.visitationid,COLUMN2 AS BloodPressure,EMRDate1 "
    '    sql = sql & "FROM EMRResults AS emrres JOIN EMRRequest emrreq "
    '    sql = sql & "ON emrres.EMRRequestID = emrreq.EMRRequestID "
    '    sql = sql & "WHERE emrdataid = 'EMR050'  AND emrcomponentid = 'EMR05002' "
    '    sql = sql & "AND EMRDate1 = (SELECT MIN(EMRDate1) FROM "
    '    sql = sql & "EMRRequest WHERE emrdataid = 'EMR050'  AND emrcomponentid = 'EMR05002' "
    '    sql = sql & "AND emrreq.visitationid = visitationid)) AS bp "
    '    sql = sql & "ON bp.visitationid = vst.VisitationID "
