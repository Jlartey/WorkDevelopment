...
Function AddStyles()
    ...
    html = html & " [name=""rptgen-report""] {width:95%;}"
...
Sub DisplayCoverLetterL(mth)
    ...
    response.write "        <div style=""font-weight:600;"">" & GetRecordField("Address") & "</div><br>"
...
Function ShowCoPayDetail(rptGen, title, spn, insSch, billMth, exWhCls)
    ...
    If HasCopay(spn) Then
        ...
        sql = Replace(sql, "@billMonthStart", " '" & billMth & "' ")
        sql = Replace(sql, "@billMonthEnd", " '" & billMth & "' ")
...
Function ShowBillDetails(rptGen, title, spn, insSch, billMth, exWhCls)
    ...
    bGrpSql = Replace(bGrpSql, "@billMonthStart", " '" & billMth & "' ")
    bGrpSql = Replace(bGrpSql, "@billMonthEnd", " '" & billMth & "' ")
    ...
    If exWhCls <> "" Then
        sql = sql & " and " & exWhCls
    End If
    sql = sql & " group by Visitation.VisitDate, Visitation.VisitationID, Patient.PatientName, InsuredPatient.InsuranceNo, InsuranceScheme.InsuranceSchemeName"
...
Function GetAllBillSQL()
    ...
    sql = sql & "     where Visitation.BillMonthID between @billMonthStart and @billMonthEnd "
    sql = sql & "  "
    sql = sql & "  "
    sql = sql & "     union all "
    sql = sql & "  "
