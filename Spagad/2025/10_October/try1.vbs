...
Sub AddNewButton()
    ...
    response.write "            }"
    response.write "        }"
    ' New: Listen for message from iframe to close modal and reload parent
    response.write "        window.addEventListener('message', function(event) {"
    response.write "            if (event.data && event.data.action === 'closeAndReload') {"
    response.write "                closeModal();"
    response.write "                location.reload();  // Reloads parent page to update table"
    response.write "            }"
    response.write "        });"
    response.write "</script>"
End Sub
