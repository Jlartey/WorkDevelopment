...
Sub DisplayCoverLetterL()
    ...
    Dim wkDy, prtFlt, totAmt, totClaim, finalBill, arrears
    Dim cellSty, cellStyB, coPayAmt, clmAmt
    Dim currentAmt, arrearsNum, finalBillNum, claimCount
    Dim totAmtStr, arrearsStr, finalBillStr, claimCountStr
    coPayAmt = GetCompanyCoPay()
    clmAmt = GetSponsorBillAmt(GetRecordField("SponsorID"))
    currentAmt = clmAmt - coPayAmt
    claimCount = GetClaimCount(GetRecordField("SponsorID"))
    arrearsNum = GetSponsorArrears(GetRecordField("SponsorID"))
    finalBillNum = currentAmt + arrearsNum

    totAmtStr = FormatNumber(CStr(currentAmt), 2, , , -1)
    claimCountStr = FormatNumber(CStr(claimCount), 0, , , -1)
    arrearsStr = FormatNumber(CStr(arrearsNum), 2, , , -1)
    finalBillStr = FormatNumber(CStr(finalBillNum), 2, , , -1)
    ...
    If arrearsNum > 0 Then
        response.write "There are arrears of <b>GHC&nbsp;" & arrearsStr & " </b> to be paid. <br>"
        response.write "Total amount for bill is <b>GHC&nbsp;" & totAmtStr & " + GHC&nbsp;" & arrearsStr & " making GHC&nbsp;" & finalBillStr & ".</b><br>"
    Else
        response.write "Total amount for bill is <b>GHC&nbsp;" & totAmtStr & " </b><br>"
    End If
    response.write "Total number of Claims are&nbsp;<b>" & claimCountStr & ".</b><br><br>"
...
Function GetSponsorArrears(sp)
    Dim rst, arrears
    Set rst = CreateObject("ADODB.Recordset")
    arrears = 0

    sql = "SELECT KeyPrefix FROM PerformVar16 WHERE PerformVar16Name = '" & sp & "' AND Description = '" & mth & "' "

    With rst
        .open sql, conn, 3, 4

        If .recordCount > 0 Then
            arrears = FormatNumber(.fields("KeyPrefix"))
        End If
    End With

    GetSponsorArrears = arrears
    Set rst = Nothing
