...
SELECT t.TreatmentID AS [Procedure Code]
     , Treatment.TreatmentName AS [Procedure Name]
     , TreatCategory.TreatCategoryName AS [Treatment Category]
     , v.SpecialistTypeID
     , COUNT(t.TreatmentID) AS [Number of cases]
     
     FROM TreatCharges AS t
     LEFT JOIN Visitation v ON t.VisitationID = v.VisitationID
    -- LEFT JOIN 
     LEFT JOIN Treatment ON Treatment.TreatmentID=t.TreatmentID
     LEFT JOIN Jobschedule ON Jobschedule.JobscheduleID=t.JobscheduleID
     LEFT JOIN Department ON Department.DepartmentID=Jobschedule.DepartmentID
     LEFT JOIN TreatCategory ON TreatCategory.TreatCategoryID=t.TreatCategoryID 
     WHERE 1=1 
	  --    AND t.TreatmentID IN ('P177', 'OP045', 'CON107', 'CON106') 
     AND t.ConsultReviewDate BETWEEN '01 Jan 2025' AND '03 Sep 2025' 
     --AND v.SpecialistTypeID = 'EME001' 011
     --AND t.MedicalStaffID = 'LMH2208009' 
     AND Treatment.TreatmentID = 'ANA004'
     AND V.SponsorID = 'SELF'

     GROUP BY t.TreatmentID, Treatment.TreatmentName, TreatCategory.TreatCategoryName, v.SpecialistTypeID  
