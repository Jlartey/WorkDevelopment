...
$reconnectDelay = 5              # Delay between reconnection attempts (seconds)
    ...
    while ($true) {
        ...
        if ($null -eq $analyzerConnection -or !$analyzerConnection.Client.Connected) {
            ...
            try {
                ...
                $lastActivityTime.Restart() # Reset activity timer on successful connection
                $global:retryCount = 0 # Reset retry count on successful connection
            }
            catch {
                Write-Log "Failed to connect to analyzer: $_" "ERROR"
        ...
        try {
            ...
            elseif ($lastActivityTime.ElapsedMilliseconds -gt $maxIdleTime) {
                Write-Log "Analyzer connection timeout (no data for $($maxIdleTime/1000) seconds)" "WARN"
                Close-Connection -client $analyzerConnection.Client -stream $analyzerConnection.Stream -connectionType "analyzer"
                $analyzerConnection = $null # Force reconnection
                continue
            }
            else {
...
$global:retryCount = 0
# Removed maxRetries limit to allow continuous operation
$reconnectDelay = 5
...
try {
    ...
    $global:retryCount++
    Write-Log "Critical error (attempt $retryCount): $_" "ERROR"
    Write-Log "Retrying in $reconnectDelay seconds..." "WARN"
    Start-Sleep -Seconds $reconnectDelay
    Start-Forwarding # Restart the forwarding process
}