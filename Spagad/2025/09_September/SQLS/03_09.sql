SELECT DISTINCT sp.SpecialistTypeName AS [Consultation Type],
    ...
    vst.VisitationID AS [Medications],
...
LEFT JOIN (
    SELECT DISTINCT visitationid, CONVERT(NVARCHAR(MAX), Column3) AS Temperature
    FROM EMRResults AS emrres
    JOIN EMRRequest emrreq ON emrres.EMRRequestID = emrreq.EMRRequestID
    WHERE emrdataid = 'EMR034' AND EMRComponentID = 'EMR03410'
) AS temp ON temp.visitationid = vst.VisitationID
LEFT JOIN (
    SELECT DISTINCT visitationid, CONVERT(NVARCHAR(MAX), Column2) AS Weight12
    FROM EMRResults AS emrres
    JOIN EMRRequest emrreq ON emrres.EMRRequestID = emrreq.EMRRequestID
    WHERE emrdataid = 'EMR034' AND EMRComponentID = 'EMR05003'
) AS wg ON wg.visitationid = vst.VisitationID
LEFT JOIN (
    SELECT DISTINCT visitationid, CONVERT(NVARCHAR(MAX), Column4) AS height
    FROM EMRResults AS emrres
    JOIN EMRRequest emrreq ON emrres.EMRRequestID = emrreq.EMRRequestID
    WHERE emrdataid = 'EMR034' AND EMRComponentID = 'EMR05004'
) AS ht ON ht.visitationid = vst.VisitationID
WHERE vst.VisitDate BETWEEN '[dateRange(0)]' AND '[dateRange(1)]'
    AND vst.PatientID <> 'P3'
    AND vst.AgeGroupID = '[ageGroup]'
    AND vst.SpecialistTypeID = '[spType]'
