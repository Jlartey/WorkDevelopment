...
Dim dateRange, SponsorID, linkType, drugId
...
If UCase(uname) = "KENNETH" And False Then
     If SponsorID = "" Then
        ...
        'PrintSponsorReport daterange
        PrintSponsorReport2 dateRange
    Else

        drugId = Request.QueryString("DrugID")
        PrintHeader dateRange
        PrintSponsorReportForDrug SponsorID, dateRange, drugId
    End If
Else
    If SponsorID = "" Then
        'AddReportHeader
...
End Function
Sub PrintSponsorReport(dateRange)
    Dim sql, rst, lastSponsorid, str, tot
    Dim cAmt, cQty, cRetQty, cIQty, cpdRetQty, cpdIQty, cddRetQty, cddIQty
    Dim globalCount, whcls, pUnit, dpt
    ...
    sql = "SELECT "
    sql = sql & "   Mydrug.SponsorID, MyDrug.DrugID, SUM(DDispQty) AS [Direct Dispenses],                "
    ...
    sql = sql & " FROM                                                                                   "
    sql = sql & "   (                                                                                    "
    sql = sql & "   SELECT DrugID,SponsorID, Qty, JobScheduleID, DispenseDate,                           "
    ...
    sql = sql & "                                                                                        "
    sql = sql & "   UNION ALL                                                                            "
    sql = sql & "   SELECT DrugID,SponsorID, DispenseAmt1 AS [Qty], JobScheduleID,                       "
    ...
    sql = sql & "          0 AS [FinalRetAmt1], 0 AS [FinalRetAmt2]                                      "
    sql = sql & "                                                                                        "
    sql = sql & "   FROM DrugSaleItems2     WHERE 1=1 AND DrugCategoryID<>'D002' AND DispenseDate " & whcls & "                     "
    sql = sql & "                                                                                        "
    sql = sql & "   UNION ALL                                                                            "
    sql = sql & "   SELECT DrugID,SponsorID, 0 AS [Qty], JobScheduleID, ReturnDate AS DispenseDate,      "
    ...
    sql = sql & "   FROM DrugReturnItems   WHERE 1=1 AND DrugCategoryID<>'D002' AND ReturnDate " & whcls & "                        "
    sql = sql & "                                                                                        "
    sql = sql & "   UNION ALL                                                                            "
    sql = sql & "   SELECT DrugID,SponsorID, 0 AS [Qty], JobScheduleID, ReturnDate AS DispenseDate,      "
    ...
    sql = sql & "   FROM DrugReturnItems2   WHERE 1=1 AND DrugCategoryID<>'D002' AND ReturnDate " & whcls & "                       "
    sql = sql & "                                                                                        "
    sql = sql & "   )       AS MyDrug                                                                    "
    sql = sql & "                                                                                        "
    sql = sql & "   GROUP BY MyDrug.DrugID, MyDrug.SponsorID                                             "
    sql = sql & "                                                                                        "
    sql = sql & "   ORDER BY SponsorID, DrugID                                                           "
    ...
    If rst.recordCount > 0 Then
        ...
        Do While Not rst.EOF
            ...
            If lastSponsorid <> rst.fields("SponsorID") Then
                If lastSponsorid <> "" Then
                    'end previous table
                    str = str & "</tbody>"
                    ...
                    str = str & "<tfoot>"
                        str = str & "<tr>"
                            ...
                            str = str & "<td class='move-right'>" & FormatNumber(cQty, 2) & "</td>"
                            str = str & "<td class='move-right'>" & FormatNumber(cAmt, 2) & "</td>"
                        str = str & "</tr>"
                    str = str & "</tfoot>"
                    str = str & "</table><br/>"
                End If
                ...
                str = str & " <colgroup>"
                    str = str & " <col class=''>"
                    str = str & " <col class=''>"
                    str = str & " <col class='' style='background-color: #f1f1f1'>"
                    str = str & " <col class='' >"
                    str = str & " <col class='' style='background-color: #f1f1f1'>"
                    str = str & " <col class='' >"
                    str = str & " <col class='' style='background-color: #f1f1f1'>"
                ...
                str = str & " </colgroup>"
                str = str & "<thead>"
                    str = str & "<tr class='sponsor'>"
                        str = str & "<th colspan='10'>" & GetComboName("Sponsor", rst.fields("SponsorID")) & "</th>"
                    str = str & "</tr>"
            ...
            str = str & "<tr>"
                str = str & "<td class='td-col drug' onclick=""run_cmd('" & rst.fields("DrugID") & "', '" & rst.fields("SponsorID") & "')"">" & rst.fields("DrugID") & "</td>"
                str = str & "<td class='td-col drug' onclick=""run_cmd('" & rst.fields("DrugID") & "', '" & rst.fields("SponsorID") & "')"">" & GetComboName("Drug", (rst.fields("DrugID"))) & "</td>"
            ...
            lastSponsorid = rst.fields("SponsorID")
            rst.MoveNext
...
'        str = str & "</table><br/>"
    ...
    str = str & "<b>Total Amt. </b><span style='font-size: 15pt; font-weight: bold;'>" & FormatNumber(tot, 2) & "</span>"
...
Sub PrintHeader(dateRange)
    response.write "<hr/>"
    response.write "<h2 style='font-size:12px;text-align:center;'>Drug Sale / Returns Report </h2>"
    response.write "<h2 style='font-size:12px;text-align:center;'>Drug Store: " & GetComboName("DrugStore", GetDrugStore()) & " </h2>"
    response.write "<hr/>"
...
Function MakeSponsorLink(drugId, spnID, dateRange)
    Dim drugName
    ...
    lnk = lnk & "&DrugID=" & drugId & "&SponsorID=" & spnID
    MakeSponsorLink = "<a href='" & lnk & "' > " & drugName & "</a>"
End Function


Sub PrintSponsorReportForDrug(SponsorID, dateRange, drugId)
    Dim sql, rst, lastSponsorid, str, tot
    Dim cAmt, cQty, cRetQty, cIQty, cpdRetQty, cpdIQty, cddRetQty, cddIQty
    ...
    sql = "SELECT "
    sql = sql & "   DrugSaleID, PatientID,Mydrug.SponsorID, MyDrug.DrugID,                               "
    ...
    sql = sql & " FROM                                                                                   "
    sql = sql & "   (                                                                                    "
    sql = sql & "   SELECT DrugSaleID, PatientID,DrugID,SponsorID, Qty, JobScheduleID, DispenseDate,     "
    sql = sql & "          Qty AS [DDispQty], 0 AS [PDispQty], 0 AS [Ret. Presc. Disp],                  "
    sql = sql & "          0 AS [Ret. Dir. Disp], finalAmt AS [FinalSaleAmt], 0 AS [FinalSaleAmt2],      "
    sql = sql & "          0 AS [FinalRetAmt1], 0 AS [FinalRetAmt2]                                      "
    sql = sql & "   FROM DrugSaleItems   WHERE 1=1 AND DrugCategoryID<>'D002' AND DispenseDate " & whcls & "                        "
    sql = sql & "                                                                                        "
    sql = sql & "   UNION                                                                                "
    sql = sql & "   SELECT DrugSaleID, PatientID,DrugID,SponsorID, DispenseAmt1 AS [Qty], JobScheduleID, "
    sql = sql & "          DispenseDate, 0 AS [DDispQty], DispenseAmt1 AS [PDispQty],                    "
    sql = sql & "          0 AS [Ret. Presc. Disp], 0 AS [Ret. Dir. Disp],                               "
    sql = sql & "          0 AS [FinalSaleAmt], DispenseAmt2 AS [FinalSaleAmt2],                         "
    sql = sql & "          0 AS [FinalRetAmt1], 0 AS [FinalRetAmt2]                                      "
    ...
    sql = sql & "                                                                                        "
    sql = sql & "   UNION                                                                                "
    sql = sql & "   SELECT DrugSaleID, PatientID,DrugID,SponsorID, 0 AS [Qty], JobScheduleID,            "
    ...
    sql = sql & "   )       AS MyDrug                                                                    "
    sql = sql & "                                                                                        "
    sql = sql & "   GROUP BY MyDrug.DrugID, MyDrug.SponsorID,DrugSaleID, PatientID                       "
    sql = sql & "                                                                                        "
    sql = sql & "   ORDER BY SponsorID, DrugID                                                           "
    ...
    If rst.recordCount > 0 Then
        ...
        Do While Not rst.EOF
            ...
            If lastSponsorid <> rst.fields("SponsorID") Then
                ...
                cAmt = 0: cQty = 0: cRetQty = 0: cIQty = 0: cpdRetQty = 0: cpdIQty = 0: cddRetQty = 0: cddIQty = 0
                'start a new table
                str = str & "<table border='1' class='table-style'>"
                str = str & "<thead >"
                    str = str & "<tr class='sponsor'>"
                        str = str & "<th colspan='10'>" & GetComboName("Sponsor", rst.fields("SponsorID")) & "</th>"
                    str = str & "</tr>"
...
'        str = str & "</table><br/>"
        ...
        str = str & "<tfoot>"
            str = str & "<tr>"
                ...
                str = str & "<td class='move-right'>" & cAmt & "</td>"
            str = str & "</tr>"
        str = str & "</tfoot>"
        str = str & "</table><br/>"
...
Function Styles()
    Dim str
     str = str & "<style>"
        ...
        str = str & "table.table-style td, table.table-style th{ font-size:11px;padding:2px 10px;}"
        str = str & ".sponsor{ background-color: #f1f1f1; font-size: larger;}"
'        str = str & ".heading{ background-image:url('images/SysTheme/N010/bgMnuListHdrColorA.bmp');}"
        str = str & ".heading{ background-color: #f1f1f1;}"
    ...
    str = str & "<script>"
        ...
        str = str & "   window.open (window.location.href + '&SponsorID=' + spnID + '&DrugID=' + drugid); " & vbCrLf
        str = str & " }"
    str = str & "</script>"
