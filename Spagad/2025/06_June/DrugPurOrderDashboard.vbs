...
printlayoutName = "DrugPurOrderDashboard"
tittle = "Medical Store Purchasing Dashboard"
MainTable = ""
...
sql = ""
sql = sql & " SELECT DrugPurOrderID, SupplierTypeID, PurchaseOrderDate, SupplierID, DrugPurOrderTypeID, DrugPurOrderStatusID, DrugCategoryID, SystemuserID, TransProcessValID"
sql = sql & " FROM DrugPurOrder " & sqlWhere & " ORDER BY  purchaseorderdate DESC  "
    ...
    With rst
        ...
        If rst.RecordCount > 0 Then
            ...
            Do While Not rst.EOF
                purID = rst.fields("DrugPurOrderID")
                supp = rst.fields("SupplierID")
                suppt = rst.fields("SupplierTypeID")
                typ = rst.fields("DrugPurOrderTypeID")
                pDt = rst.fields("PurchaseOrderDate")
                purfr = rst.fields("DrugPurOrderStatusID")
                transproStage = rst.fields("TransProcessValID")
                cat = rst.fields("DrugCategoryID")
                usr = rst.fields("SystemuserID")
                pos = rst.AbsolutePosition

                href = "wpgDrugPurOrder.asp?PageMode=ProcessSelect&DrugPurOrderID=" & purID
                phref = "wpgPrtPrintLayoutAll.asp?PrintLayoutName=PurchaseOrderPrint&PositionForTableName=DrugPurOrder&DrugPurOrderID=" & purID
                ...
                response.write "                  </div></td>"
                response.write "                  <td><div class='df fc fs'>"
                response.write "                    <div class='p5'><span class='pr5 bold'>Type:</span>" & GetComboName("DrugPurOrderType", typ) & "</div>"
                response.write "                    <div class='p5'><span class='pr5 bold'>Item Category:</span>" & GetComboName("DrugCategory", cat) & "</div>"
                response.write "                    <div class='p5 df rlt'><span class='pr5 bold wn'>Order Items:</span>"
                    ...
                    If UCase(transproStage) <> UCase("DrugPurOrderPro-T001") Then
                ...
                If Approved(purID, "DrugPurOrderPro-T002") Then
                    response.write "                    <div class='p5'><span class='pr5 bold'>Approved by:</span>" & getUserAppv(purID, "DrugPurOrderPro-T002") & "</div>"
                Else
                    ...
                    'by Mike
                     If HasAccessRight(uName, "frm" & sTb2, "New") And Glob_HasTransProcessAccess(sTb2, uName, "T001", "T002") Then
                        ...
                        lnkUrl = "wpgDrugPurOrderPro.asp?PageMode=AddNew&TransProcessVal2ID=DrugPurOrderPro-T002&PullupData=DrugPurOrderID||" & purID
                ...
                If RequiresCEOApproval(purID) Then
                    If Approved(purID, "DrugPurOrderPro-T011") Then
                        response.write "                    <div class='p5'><span class='pr5 bold'>Approved by:</span>" & getUserAppv(purID, "DrugPurOrderPro-T011") & "</div>"
                    Else
                        ...
                        sTb2 = "DrugPurOrderPro"
                        'by Mike
                         If HasAccessRight(uName, "frm" & sTb2, "New") And Glob_HasTransProcessAccess(sTb2, uName, "T002", "T011") And Approved(purID, "DrugPurOrderPro-T002") Then
                            lnkText = "Approve Purchase Request"
                            lnkUrl = "wpgDrugPurOrderPro.asp?PageMode=AddNew&TransProcessVal2ID=DrugPurOrderPro-T011&PullupData=DrugPurOrderID||" & purID
                            response.write Addlnk(lnkText, lnkUrl, "bss")
                         End If
                        response.write "                  </div></div>"
                    End If
                Else
                    If Approved(purID, "DrugPurOrderPro-T003") Then
                        response.write "                    <div class='p5'><span class='pr5 bold'>Approved by:</span>" & getUserAppv(purID, "DrugPurOrderPro-T003") & "</div>"
                    Else
                        response.write "                    <div class='df w100'><div class='p5 f1'><span class='datastat pnd'>Pending Administrator Approval</span></div><div class='f1 df f-end'>"
                        sTb2 = "DrugPurOrderPro"
                        'by Mike
                         If HasAccessRight(uName, "frm" & sTb2, "New") And Glob_HasTransProcessAccess(sTb2, uName, "T002", "T003") And Approved(purID, "DrugPurOrderPro-T002") Then
                            lnkText = "Approve Purchase Request"
                            lnkUrl = "wpgDrugPurOrderPro.asp?PageMode=AddNew&TransProcessVal2ID=DrugPurOrderPro-T003&PullupData=DrugPurOrderID||" & purID
                            response.write Addlnk(lnkText, lnkUrl, "bss")
                         End If
                        response.write "                  </div></div>"
                    End If
                ...
                If RequiresCEOApproval(purID) Then
                    If Approved(purID, "DrugPurOrderPro-T004") Then
                        response.write "                    <div class='p5'><span class='pr5 bold'>Approved by:</span>" & getUserAppv(purID, "DrugPurOrderPro-T004") & "</div>"
                    Else
                        ...
                        'by Mike
                         If HasAccessRight(uName, "frm" & sTb2, "New") And Glob_HasTransProcessAccess(sTb2, uName, "T011", "T004") Then
                            lnkText = "Approve Purchase Request"
                            lnkUrl = "wpgDrugPurOrderPro.asp?PageMode=AddNew&TransProcessVal2ID=DrugPurOrderPro-T004&PullupData=DrugPurOrderID||" & purID
                            response.write Addlnk(lnkText, lnkUrl, "bss")
                         End If
                        response.write "                  </div></div>"
                    End If
...
Sub DisplayAcceptedItems(reqId, pos)
    Dim rst, sql
    Set rst = CreateObject("ADODB.Recordset")
    sql = "select da.*, d.DrugName from IncomingDrugItems da, Drug d where d.DrugID=da.DrugID and da.DrugPurOrderID='" & reqId & "' "
    sql = sql & " order by d.DrugName "

    With rst
        rst.open qryPro.FltQry(sql), conn, 3, 4
        If rst.RecordCount > 0 Then
            srvID = GetComboNameFld("IncomingDrug", reqId, "IncomingDrugID")
            iref = "wpgPrtPrintLayoutAll.asp?PrintLayoutName=SRVReportMS&PositionForTableName=IncomingDrug&IncomingDrugID=" & rst.fields("IncomingDrugID")
            ref = "wpgIncomingDrug.asp?PageMode=ProcessSelect&PullupData=DrugPurOrderID||" & reqId
            rst.movefirst
            ...
            response.write "                    <div class='p5 df rlt'><span class='pr5 bold'>Accepted Items:</span>"
                ...
                response.write "                <div>" & Addlnk("Print SRV", iref, "bs") & "</div>"
            ...
            Loop
                ...
                response.write "                 </div>"
                response.write "                </div></div>"
                If SupplyNotComplete(reqId) Then 'And Approved(reqID, "DrugPurOrderPro-T001") Then 'Mike
                    sTb2 = "IncomingDrug"
                    'by Mike
                     If HasAccessRight(uName, "frm" & sTb2, "New") Then
                        lnkText = "Accept remaining Items"
                        lnkUrl = "wpg" & sTb2 & ".asp?PageMode=AddNew&PullupData=DrugPurOrderID||" & reqId
                        response.write Addlnk(lnkText, lnkUrl, "bss")
                     End If
                End If
            response.write "                  </div>"
        Else
            ...
            sTb2 = "IncomingDrug"
            'If Approved(reqID, "DrugPurOrderPro-T002") Then
            'by Mike
                 If HasAccessRight(uName, "frm" & sTb2, "New") Then
...
Sub DisplayRequestedItems(reqId)
    Dim rst, sql
    Set rst = CreateObject("ADODB.Recordset")
    sql = "select dr.*, d.DrugName, u.UnitOfMeasureName from DrugPurOrderItems dr, Drug d, UnitOfMeasure u Where d.DrugID=dr.DrugID And u.UnitOfMeasureID=d.UnitOfMeasureID "
    sql = sql & " And dr.DrugPurOrderID='" & reqId & "' order by d.DrugName "
    With rst
        rst.open qryPro.FltQry(sql), conn, 3, 4
        If rst.RecordCount > 0 Then
            rst.movefirst
...
Function RequiresCEOApproval(purID)
    ...
    If rst.RecordCount > 0 Then
        ...
        sql = sql & " from ( "
        sql = sql & "     select OrderAmount2 from DrugPurOrderItems where DrugPurOrderID='" & rst.fields("DrugPurOrderID") & "' "
        sql = sql & "     union all select OrderAmount2 from DrugPurOrderItems2 where DrugPurOrderID='" & rst.fields("DrugPurOrderID") & "' "
...
Function SupplyNotComplete(reqId)
    ...
    Set rst = CreateObject("ADODB.Recordset")
    With rst
        sql = "Select Sum(ReturnQty) tot from IncomingDrugItems "
        sql = sql & " WHERE DrugPurOrderID='" & reqId & "' "
        .maxrecords = 1
        .open qryPro.FltQry(sql), conn, 3, 4
        If .RecordCount > 0 Then
            ot = rst.fields("tot")
...
Function getReqQuantity(reqId)
    ...
    Set rst = CreateObject("ADODB.Recordset")
    With rst
        sql = " Select Sum(OrderQuantity) as tot from DrugPurOrderItems "
        sql = sql & " WHERE DrugPurOrderID='" & reqId & "' "
        .maxrecords = 1
        .open qryPro.FltQry(sql), conn, 3, 4
        If .RecordCount > 0 Then
...
Function Approved(purID, transproStage)
    ...
    Set rst = CreateObject("ADODB.Recordset")
    With rst
        sql = "Select top 1 DrugPurOrderProID from DrugPurOrderPro "
        sql = sql & " where DrugPurOrderID='" & purID & "' And TransProcessVal2ID='" & transproStage & "'"
        .maxrecords = 1
        .open qryPro.FltQry(sql), conn, 3, 4
        If .RecordCount > 0 Then
            ot = True
...
Function getUserAppv(purID, transproStage)
    ...
    Set rst = CreateObject("ADODB.Recordset")
    With rst
        sql = "Select top 1 SystemuserID from DrugPurOrderPro "
        sql = sql & " where DrugPurOrderID='" & purID & "' And TransProcessVal2ID='" & transproStage & "'"
        .maxrecords = 1
        .open qryPro.FltQry(sql), conn, 3, 4
        If .RecordCount > 0 Then
...
Function GetSupWrkmnt()
    ...
    Set rst = CreateObject("ADODB.Recordset")
    With rst
        sql = "Select top 1 WorkingMonthID from DrugPurOrder "
...
Function DetailSelectorFilter(itmID, itmName, table, id)
    ...
    If UCase(table) = UCase("DrugStore") Or UCase(table) = UCase("WorkingMonth") Then
...
Sub TableEX()
    ...
    'by Mike
     If HasAccessRight(uName, "frm" & sTb2, "New") Then
        response.write "        <div class='df f1 plr25 mlr25'> " & Addlnk("Add New Purchase Order", "wpgDrugPurOrder.asp?PageMode=AddNew", "bp") & "</div>"
     End If
    response.write "        </div>"
