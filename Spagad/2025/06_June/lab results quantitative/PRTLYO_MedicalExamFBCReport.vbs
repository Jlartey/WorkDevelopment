...
sql = sql & " SELECT i.LabRequestID, i.LabTestID, i.RequestDate, i.VisitationID, i.PatientID, i.GenderID, "
sql = sql & " i.BranchID, [L0460].Column2 AS WBC, [L0468].Column2 AS RBC, [L0467].Column2 AS HB, [L0469].Column2 AS HCT, "
sql = sql & " [L0470].Column2 AS MCV, [L0471].Column2 AS MCH, [L0472].Column2 AS MCHC, [L0475].Column2 AS PLT, [L0200].Column2 AS RDWSD, "
sql = sql & " [L0201].Column2 AS RDWCV, [L0202].Column2 AS PDW, [L0476].Column2 AS MPV, [L0204].Column2 AS PLCR, [L0323].Column2 AS PCT, "
sql = sql & " [L0199].Column2 AS NEUTGRAN, [L0196].Column2 AS [NEUTGRANN], [L0461].Column2 AS [LYM], [L0464].Column2 AS [LYMPH], "
sql = sql & " [L0317].Column2 AS [MONO], [L0318].Column2 AS [MONOO], [L0319].Column2 AS [EO], [L0320].Column2 AS [EOO], [L0321].Column2 AS [BASO], "
sql = sql & " [L0322].Column2 AS [BASOO], [L0719].Column2 AS [MID], [L0720].Column2 AS [MIDD], "
'LabResults Reference Ranges - High or Low
sql = sql & " [L0460].Column1 AS WBC_RefR, [L0468].Column1 AS RBC_RefR, [L0467].Column1 AS HB_RefR, [L0469].Column1 AS HCT_RefR, "
sql = sql & "[L0470].Column1 AS MCV_RefR, [L0471].Column1 AS MCH_RefR, [L0472].Column1 AS MCHC_RefR, [L0475].Column1 AS PLT_RefR,  "
   sql = sql & " [L0200].Column1 AS RDWSD_RefR, [L0201].Column1 AS RDWCV_RefR, "
   sql = sql & " [L0202].Column1 AS PDW_RefR, "
   sql = sql & " [L0476].Column1 AS MPV_RefR, "
   sql = sql & " [L0204].Column1 AS PLCR_RefR, "
   sql = sql & " [L0323].Column1 AS PCT_RefR, "
   sql = sql & " [L0199].Column1 AS NEUTGRAN_RefR, "
   sql = sql & " [L0196].Column1 AS NEUTGRANN_RefR, "
   sql = sql & " [L0461].Column1 AS LYM_RefR, "
   sql = sql & " [L0464].Column1 AS LYMPH_RefR, "
   sql = sql & " [L0317].Column1 AS MONO_RefR, "
   sql = sql & " [L0318].Column1 AS MONOO_RefR, "
   sql = sql & " [L0319].Column1 AS EO_RefR, "
   sql = sql & " [L0320].Column1 AS EOO_RefR, "
   sql = sql & " [L0321].Column1 AS BASO_RefR, "
   sql = sql & " [L0322].Column1 AS BASOO_RefR, "
   sql = sql & " [L0719].Column1 AS MID_RefR, "
   sql = sql & " [L0720].Column1 AS MIDD_RefR "
...
sql = sql & " LEFT JOIN Visitation v ON i.VisitationID = v.VisitationID "
   ...
   sql = sql & " LEFT JOIN LabResults AS [L0720] ON i.LabRequestID = [L0720].LabRequestID "
    ...
    sql = sql & " SELECT i2.LabRequestID, i2.LabTestID, i2.RequestDate, i2.VisitationID, i2.PatientID, "
    sql = sql & " i2.GenderID, i2.BranchID, [L0460].Column2 AS WBC, [L0468].Column2 AS RBC, [L0467].Column2 AS HB, "
    sql = sql & " [L0469].Column2 AS HCT, [L0470].Column2 AS MCV, [L0471].Column2 AS MCH, [L0472].Column2 AS MCHC,  "
    sql = sql & " [L0475].Column2 AS PLT, [L0200].Column2 AS RDWSD, [L0201].Column2 AS RDWCV, [L0202].Column2 AS PDW,  "
    sql = sql & " [L0476].Column2 AS MPV, [L0204].Column2 AS PLCR, [L0323].Column2 AS PCT, [L0199].Column2 AS NEUTGRAN,  "
    sql = sql & " [L0196].Column2 AS [NEUTGRANN], [L0461].Column2 AS [LYM], [L0464].Column2 AS [LYMPH],  "
    sql = sql & " [L0317].Column2 AS [MONO], [L0318].Column2 AS [MONOO], [L0319].Column2 AS [EO],  "
    sql = sql & " [L0320].Column2 AS [EOO], [L0321].Column2 AS [BASO], [L0322].Column2 AS [BASOO],  "
    sql = sql & " [L0719].Column2 AS [MID], [L0720].Column2 AS [MIDD], "
   ...
   sql = sql & " LEFT JOIN LabResults AS [L0720] ON i2.LabRequestID = [L0720].LabRequestID "
    ...
    End If
        ...
        sql = sql & " AND v.SpecialistTypeID IN ('B001-0200','B002-0208','B003-0208') "
         ...
         End If
sql = sql & " ) "
sql = sql & " SELECT LabRequestID, LabTestID,BranchID, RequestDate,VisitationID,PatientID, GenderID,WBC,RBC,HB,HCT, MCV,MCH,MCHC,PLT,RDWSD, RDWCV,PDW, MPV,PLCR,PCT,NEUTGRAN, [NEUTGRANN],[LYM],[LYMPH],[MONO],[MONOO], [EO], [EOO], [BASO],[BASOO],[MID],[MIDD], "
'Reference Range columns
sql = sql & " WBC_RefR, RBC_RefR, HB_RefR,  HCT_RefR, "
sql = sql & " MCV_RefR, MCH_RefR, MCHC_RefR, PLT_RefR, RDWSD_RefR,  "
sql = sql & " RDWCV_RefR, "
sql = sql & " PDW_RefR, "
sql = sql & " MPV_RefR, "
sql = sql & " PLCR_RefR, "
sql = sql & " PCT_RefR, "
sql = sql & " NEUTGRAN_RefR, "
sql = sql & " NEUTGRANN_RefR, "
sql = sql & " LYM_RefR, "
sql = sql & " LYMPH_RefR, "
sql = sql & " MONO_RefR, "
sql = sql & " MONOO_RefR, "
...
sql = sql & " MID_RefR, "
sql = sql & " MIDD_RefR "
sql = sql & " FROM LabReportRange ORDER BY RequestDate  "
...
response.write "<a class=""btn btn-primary"" href=""wpgPrtPrintLayoutAll.asp?PrintLayoutName=MedicalExamFBCReport&PositionForTableName=WorkingDay&dptID=" & dpt & "&secID=" & secid & "&genID=" & genID & "&branID=" & branid & "&yrID=" & yrid & " "">Refresh</a>"
response.write "        </td>"
...
response.write "<th>[MID%]</th>"
' response.write "<th>WBC Ref. R</th>"
response.write "</tr>"
...
With rst
    ...
    If rst.RecordCount > 0 Then
        ...
        '   wbcRefR = rst.fields("WBC_RefR")
        '   SetPageMessages wbcRefR
...
' ' Sample range stored as a string from the database
' Dim rangeString, lowerBound, upperBound, value, parsedBounds

' ' Example of the range string from the database
' ' rangeString = "[0.20 - 0.74]"
' wbcRefR = rst.fields("WBC_RefR")

' ' Set the value to be checked
' ' value = 0.5
' wbcValue = CDbl(rst.fields("WBC"))



' ' Parse the range string to extract numerical bounds
' parsedBounds = ParseRange(wbcRefR)



' ' Assign extracted bounds to lower and upper bound variables
' lowerBound = parsedBounds(0)
' upperBound = parsedBounds(1)
' ' lowerBound = CDbl(parsedBounds(0))
' ' upperBound = CDbl(parsedBounds(1))


' ' Check if the value falls within the extracted range
' If wbcValue >= lowerBound And wbcValue <= upperBound Then
'     response.write "<td>" & rst.fields("WBC") & "</td>"
' Else
'     response.write "<td style='background-color: #ff6699;'>" & rst.fields("WBC") & "</td>"
    
' End If
            wbcRefR = rst.fields("WBC_RefR")
            RBC_RefR = rst.fields("RBC_RefR")
            HB_RefR = rst.fields("HB_RefR")
            HCT_RefR = rst.fields("HCT_RefR")
            MCV_RefR = rst.fields("MCV_RefR")
            MCH_RefR = rst.fields("MCH_RefR")
            MCHC_RefR = rst.fields("MCHC_RefR")
            PLT_RefR = rst.fields("PLT_RefR")
            RDWSD_RefR = rst.fields("RDWSD_RefR")
            RDWCV_RefR = rst.fields("RDWCV_RefR")
            PDW_RefR = rst.fields("PDW_RefR")
            MPV_RefR = rst.fields("MPV_RefR")
            PLCR_RefR = rst.fields("PLCR_RefR")
            PCT_RefR = rst.fields("PCT_RefR")
            NEUTGRAN_RefR = rst.fields("NEUTGRAN_RefR")
            NEUTGRANN_RefR = rst.fields("NEUTGRANN_RefR")
            LYM_RefR = rst.fields("LYM_RefR")
            LYMPH_RefR = rst.fields("LYMPH_RefR")
            MONO_RefR = rst.fields("MONO_RefR")
            MONOO_RefR = rst.fields("MONOO_RefR")
            EO_RefR = rst.fields("EO_RefR")
            EOO_RefR = rst.fields("EOO_RefR")
            BASO_RefR = rst.fields("BASO_RefR")
            BASOO_RefR = rst.fields("BASOO_RefR")
            MID_RefR = rst.fields("MID_RefR")
            MIDD_RefR = rst.fields("MIDD_RefR")

            If wbcRefR = "L" Or wbcRefR = "H" Then
                response.write "<td style='background-color: #ff6699;'>" & rst.fields("WBC") & "</td>"
            Else
            response.write "<td>" & rst.fields("WBC") & "</td>"
            End If

            If RBC_RefR = "L" Or RBC_RefR = "H" Then
                response.write "<td style='background-color: #ff6699;'>" & rst.fields("RBC") & "</td>"
            Else
            response.write "<td>" & rst.fields("RBC") & "</td>"
            End If
            
            If HB_RefR = "L" Or HB_RefR = "H" Then
                response.write "<td style='background-color: #ff6699;'>" & rst.fields("HB") & "</td>"
            Else
            response.write "<td>" & rst.fields("HB") & "</td>"
            End If

            If HCT_RefR = "L" Or HCT_RefR = "H" Then
                response.write "<td style='background-color: #ff6699;'>" & rst.fields("HCT") & "</td>"
            Else
            response.write "<td>" & rst.fields("HCT") & "</td>"
            End If

            If MCV_RefR = "L" Or MCV_RefR = "H" Then
                response.write "<td style='background-color: #ff6699;'>" & rst.fields("MCV") & "</td>"
            Else
            response.write "<td>" & rst.fields("MCV") & "</td>"
            End If
            
            If MCH_RefR = "L" Or MCH_RefR = "H" Then
                response.write "<td style='background-color: #ff6699;'>" & rst.fields("MCH") & "</td>"
            Else
            response.write "<td>" & rst.fields("MCH") & "</td>"
            End If
            
            If MCHC_RefR = "L" Or MCHC_RefR = "H" Then
                response.write "<td style='background-color: #ff6699;'>" & rst.fields("MCHC") & "</td>"
            Else
            ...
            If PLT_RefR = "L" Or PLT_RefR = "H" Then
                response.write "<td style='background-color: #ff6699;'>" & rst.fields("PLT") & "</td>"
            Else
            ...
            If RDWSD_RefR = "L" Or RDWSD_RefR = "H" Then
                response.write "<td style='background-color: #ff6699;'>" & rst.fields("RDWSD") & "</td>"
            Else
                 response.write "<td>" & rst.fields("RDWSD") & "</td>"
            End If


            If RDWCV_RefR = "L" Or RDWCV_RefR = "H" Then
                response.write "<td style='background-color: #ff6699;'>" & rst.fields("RDWCV") & "</td>"
            Else
            response.write "<td>" & rst.fields("RDWCV") & "</td>"
            End If


            If PDW_RefR = "L" Or PDW_RefR = "H" Then
                response.write "<td style='background-color: #ff6699;'>" & rst.fields("PDW") & "</td>"
            Else
            response.write "<td>" & rst.fields("PDW") & "</td>"
            End If
            
            
            If MPV_RefR = "L" Or MPV_RefR = "H" Then
                response.write "<td style='background-color: #ff6699;'>" & rst.fields("MPV") & "</td>"
            Else
            ...
            If PLCR_RefR = "L" Or PLCR_RefR = "H" Then
                response.write "<td style='background-color: #ff6699;'>" & rst.fields("PLCR") & "</td>"
            Else
            ...
            If PCT_RefR = "L" Or PCT_RefR = "H" Then
                response.write "<td style='background-color: #ff6699;'>" & rst.fields("PCT") & "</td>"
            Else
            response.write "<td>" & rst.fields("PCT") & "</td>"
            End If
            
            If NEUTGRAN_RefR = "L" Or NEUTGRAN_RefR = "H" Then
                response.write "<td style='background-color: #ff6699;'>" & rst.fields("NEUTGRAN") & "</td>"
            Else
            response.write "<td>" & rst.fields("NEUTGRAN") & "</td>"
            End If
            

            If NEUTGRANN_RefR = "L" Or NEUTGRANN_RefR = "H" Then
                response.write "<td style='background-color: #ff6699;'>" & rst.fields("NEUTGRANN") & "</td>"
            Else
            response.write "<td>" & rst.fields("NEUTGRANN") & "</td>"
            End If
            

            If LYM_RefR = "L" Or LYM_RefR = "H" Then
                response.write "<td style='background-color: #ff6699;'>" & rst.fields("LYM") & "</td>"
            Else
            response.write "<td>" & rst.fields("LYM") & "</td>"
            End If


            If LYMPH_RefR = "L" Or LYMPH_RefR = "H" Then
                response.write "<td style='background-color: #ff6699;'>" & rst.fields("LYMPH") & "</td>"
            Else
            response.write "<td>" & rst.fields("LYMPH") & "</td>"
            End If
            

            If MONO_RefR = "L" Or MONO_RefR = "H" Then
                response.write "<td style='background-color: #ff6699;'>" & rst.fields("MONO") & "</td>"
            Else
            response.write "<td>" & rst.fields("MONO") & "</td>"
            End If
            
            If MONOO_RefR = "L" Or MONOO_RefR = "H" Then
                response.write "<td style='background-color: #ff6699;'>" & rst.fields("MONOO") & "</td>"
            Else
            response.write "<td>" & rst.fields("MONOO") & "</td>"
            End If

            If EO_RefR = "L" Or EO_RefR = "H" Then
                response.write "<td style='background-color: #ff6699;'>" & rst.fields("EO") & "</td>"
            Else
            response.write "<td>" & rst.fields("EO") & "</td>"
            End If
            
            If EOO_RefR = "L" Or EOO_RefR = "H" Then
                response.write "<td style='background-color: #ff6699;'>" & rst.fields("EOO") & "</td>"
            Else
            response.write "<td>" & rst.fields("EOO") & "</td>"
            End If

            If BASO_RefR = "L" Or BASO_RefR = "H" Then
                response.write "<td style='background-color: #ff6699;'>" & rst.fields("BASO") & "</td>"
            Else
            response.write "<td>" & rst.fields("BASO") & "</td>"
            End If
            
            If BASOO_RefR = "L" Or BASOO_RefR = "H" Then
                response.write "<td style='background-color: #ff6699;'>" & rst.fields("BASOO") & "</td>"
            Else
            response.write "<td>" & rst.fields("BASOO") & "</td>"
            End If
            
            If MID_RefR = "L" Or MID_RefR = "H" Then
                response.write "<td style='background-color: #ff6699;'>" & rst.fields("MID") & "</td>"
            Else
            response.write "<td>" & rst.fields("MID") & "</td>"
            End If

            If MIDD_RefR = "L" Or MIDD_RefR = "H" Then
                response.write "<td style='background-color: #ff6699;'>" & rst.fields("MIDD") & "</td>"
            Else
            response.write "<td>" & rst.fields("MIDD") & "</td>"
            End If
            
            ' response.write "<td>" & rst.fields("WBC_RefR") & "</td>"
            response.write "</tr>"
...
response.write "                    title: '" & GetComboName("LabTest", dpt) & " Labtest-" & FormatDate(Now) & "' "
response.write "                },"
response.write "                {"
...
End Sub
Sub MedicalExaminationLabResultsAllLFT()
...
sql = sql & " LEFT JOIN Visitation v ON i.VisitationID = v.VisitationID "
   ...
   sql = sql & " LEFT JOIN LabResults AS [L0399] ON i.LabRequestID = [L0399].LabRequestID "
    ...
    sql = sql & " Where i.LabTestID = '" & lftid & "' "
        If Len(branid) > 0 Then
        sql = sql & " AND i.BranchID= '" & lftbranid & "' "
    End If
    If Len(genID) > 0 Then
        sql = sql & " AND i.GenderID= '" & lftgenid & "' "
    End If
        sql = sql & " AND i.SponsorID IN ('S571','S592','S593', 'S594','S657','VRA') "
        sql = sql & " AND v.SpecialistTypeID IN ('B001-0200','B002-0208','B003-0208') "
        If Len(yrid) > 0 Then
   ...
   sql = sql & " LEFT JOIN LabResults AS [L0399] ON i2.LabRequestID = [L0399].LabRequestID "
        sql = sql & " AND [L0399].TestComponentID = 'L0399' "
    sql = sql & " Where i2.LabTestID = '" & lftid & "' "
    If Len(branid) > 0 Then
        sql = sql & " AND i2.BranchID='" & lftbranid & "' "
    End If
    If Len(genID) > 0 Then
        sql = sql & " AND i2.GenderID='" & lftgenid & "' "
    End If
...
'response.write "<h4>AME " & GetComboName("WorkingYear", yrid) & " LAB TEST RECORDS &gt;&gt;&gt; " & GetComboName("LabTest", dpt) & "</h4>"
'
...
sql6 = ""
sql6 = sql6 & " SELECT LabTestID, LabTestName FROM LabTest WHERE LabTestID = 'L010'  "
...
sql9 = ""
sql9 = sql9 & " SELECT BranchID FROM Branch WHERE BranchID IN ('B001','B002','B003','B004') "
...
response.write "<select  id='cIDLft1'>"
...
response.write "<select  id='cIDLft3'>"
...
response.write "<select  id='cIDLft4'>"
...
'Year COMBO STARTS
response.write "<select  id='cIDLft5'>"
...
response.write "<a class=""btn btn-primary"" onClick=""reloadLft()"">Apply Filter(s)</a>"
response.write "        </td>"
...
response.write "</table>"
response.write "</div>"
response.write "                    <table id=""lftTab"" class=""table table-striped table-bordered""  width=""100%"">"
response.write "                        <thead class=""beautiful-green-header"">"
response.write "<tr>"
response.write "<th>#</th>"
response.write "<th>LabRequestID</th>"
...
response.write "<script>"
response.write "function reload2() {"
response.write "    let selected = document.getElementById('cIDLft1').value;"
...
response.write "</script>"
response.write "<script>"
response.write "function reloadLft() {"
response.write "    let selected = document.getElementById('cIDLft1').value;"
'response.write "    let selected2 = document.getElementById('cID2').value;"
response.write "    let selected3 = document.getElementById('cIDLft3').value;"
response.write "    let selected4 = document.getElementById('cIDLft4').value;"
response.write "    let selected5 = document.getElementById('cIDLft5').value;"
'response.write "    let sat = document.getElementById('statusFilter').value;"
response.write "    let url = 'wpgPrtPrintLayoutAll.asp?PrintLayoutName=MedicalExamLabResultRCP';"
response.write "    url += '&PositionForTableName=WorkingDay&lftID=' + selected + '&lftgenID=' + selected3 + '&lftbranID=' + selected4 + '&yrID=' + selected5;"
response.write "    window.location.href = url;"
response.write "}"
...
response.write "        var dataTable = $('#lftTab').DataTable({"
response.write "        lengthMenu: [[10, 25, 50, 100, -1], [10, 25, 50, 100, ""All""]],"
response.write "        dom: 'lBfrtip',"
response.write "            pageLength: 10,"
response.write "            paging: true, "
...
response.write "<header class=""header_part"">"
response.write "<h4>AME " & GetComboName("WorkingYear", yrid) & " LAB TEST RECORDS &gt;&gt;&gt;FULL BLOOD COUNT [FBC]</h4>"
...
Dim rst6, rst7, rst8, rst9, rst10, sql6, sql7, sql8, sql9, sql10
         ...
         Set rst10 = CreateObject("ADODB.Recordset")
sql6 = ""
sql6 = sql6 & " SELECT LabTestID, LabTestName FROM LabTest WHERE LabTestID = 'L405'  "
...
' Function to parse the range string and extract bounds
Function ParseRange(rangeStr)

SetPageMessages "I am running!"
    Dim bounds(1), startPos, endPos
    
    ' Remove the square brackets and split the range string
    rangeStr = Replace(wbcRefR, "[", "")
    rangeStr = Replace(wbcRefR, "]", "")
    
    ' Split the range by the delimiter ' - '
    bounds = Split(wbcRefR, " - ")
    
    ' Convert the string bounds to numerical values
    ParseRange = Array(CDbl(bounds(0)), CDbl(bounds(1)))
SetPageMessages bounds(0)
End Function
