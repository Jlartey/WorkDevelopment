...
Function OnActiveAdmission(pat, vst)
  ...
  If .RecordCount > 0 Then
    ...
    ' msg = msg & " [" & GetComboName("Visitation", rst.fields("visitationid")) & "] [" & rst.fields("admissionid") & "]]"
...
Function HasPatientPaidByVal(amtTbl, amtfld, amtval, kyFld, recNo, recTyp, pat, kVl, kTb, bDt, vst, dDt)
  ...
  Else
    totAmt = 0
    vld = True
    'amt = Trim(request("inp" & amtFld))
    amt = CStr(amtval)
    totAmt = GetAdmissionAmount(pat, recNo, vst, dDt)
    'SetPageMessages totAmt
    If Len(amt) > 0 Then
    ...
    If vld Then
     ...
     '     rec = Trim(arr2(num2))
     '     If Len(rec) > 0 Then
     '       pid = Trim(GetComboNameFld("Receipt", rec, "PatientID"))
     '       If UCase(pid) = UCase(Trim(pat)) Then
     '         balAmt = GetComboNameFld("Receipt", rec, "ReceiptAmount3")
     '         If IsNumeric(balAmt) Then
     '           totBal = totBal + CDbl(balAmt)
     '         End If
     '         currAmt = GetUsedByCurrTrans(rec, kVl, kTb)
     '         totBal = totBal + currAmt
     '       Else
     '         SetPageMessages "Receipt No. [" & UCase(rec) & "] is not for Patient ->[" & pat & "] " & GetComboName("Patient", pat) & ". It is for [" & pid & "] " & GetComboName("Patient", pid)
     '       End If
     '     End If
     '   Next
     ' End If
