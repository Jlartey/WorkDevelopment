...
Sub Entries()
    ...
    response.write "                url = url + '&VisitationID=' + encodeURIComponent('" & visitationId & "');"
    response.write "                "
    ...
    response.write "        });"
    response.write "      intervention.value = '';"
    response.write "      treatmentValue.value = '';"
    response.write "     }"
    response.write "   })"
...
Sub DisplayTable()
    Dim count, sql, rst, treatmentDate, treatmentValue, cTime
    count = 1
    
    
    Set rst = CreateObject("ADODB.Recordset")

    sql =  "SELECT * FROM PerformVar11 WHERE CHARINDEX('||', PerformVar11Name) > 0 "
    sql = sql & "AND SUBSTRING(PerformVar11Name, CHARINDEX('||', PerformVar11Name) + 2, LEN(PerformVar11Name)) = ' " & visitationId &" ' "
    sql = sql & "ORDER BY PerformVar11ID DESC"
