...
If Len(nm) > 0 Then
    ...
    Dim nameParts, i
    nameParts = Split(Trim(nm), " ")
    
    ' Build a condition for each word
    sqlWhCls = sqlWhCls & " ("
    For i = 0 To UBound(nameParts)
        If Len(Trim(nameParts(i))) > 0 Then ' Skip empty parts
            If i > 0 Then
                sqlWhCls = sqlWhCls & " AND "
            End If
            sqlWhCls = sqlWhCls & " Patient.PatientName LIKE '%" & Trim(nameParts(i)) & "%'"
        End If
    Next
    sqlWhCls = sqlWhCls & ")"
End If
