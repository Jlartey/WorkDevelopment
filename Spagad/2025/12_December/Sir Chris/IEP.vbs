...
If isPost Then
    ...
    If errors = "" Then
        ...
        ' Use positional parameters with NON-EMPTY parameter names (e.g., "@p1", "@p2", etc.)
        ' This avoids the OLE DB bug that inserts "DEFAULT" when the name is empty string
        cmd.CommandText = _
        ...
        cmd.Parameters.Append cmd.CreateParameter("@p1", 200, 1, 50, VisitationID)
        cmd.Parameters.Append cmd.CreateParameter("@p2", 200, 1, 50, DrugID)
        cmd.Parameters.Append cmd.CreateParameter("@p3", 200, 1, 50, Dose)
        cmd.Parameters.Append cmd.CreateParameter("@p4", 200, 1, 50, Frequency)
        cmd.Parameters.Append cmd.CreateParameter("@p5", 200, 1, 50, Route)
        cmd.Parameters.Append cmd.CreateParameter("@p6", 200, 1, 255, AppDrug)
        cmd.Parameters.Append cmd.CreateParameter("@p7", 200, 1, 255, AppDose)
        cmd.Parameters.Append cmd.CreateParameter("@p8", 200, 1, 255, AppFreq)
        cmd.Parameters.Append cmd.CreateParameter("@p9", 200, 1, 255, AppRoute)
        cmd.Parameters.Append cmd.CreateParameter("@p10", 200, 1, 255, TherDup)
        cmd.Parameters.Append cmd.CreateParameter("@p11", 200, 1, 4000, Comments)
        cmd.Parameters.Append cmd.CreateParameter("@p12", 200, 1, 4000, InterventionOutcome)

        On Error Resume Next
        cmd.Execute
        If Err.Number <> 0 Then
            errors = "Database error: " & Server.HTMLEncode(Err.Description)
            Err.Clear
        Else
            successMessage = "Therapy Appropriateness Audit saved successfully."
            ' Clear fields after success
            DrugID = "": Dose = "": Frequency = "": Route = ""
            AppDrug = "": AppDose = "": AppFreq = "": AppRoute = "": TherDup = ""
            Comments = "": InterventionOutcome = ""
        End If
        On Error Goto 0
...
Dim options, freqOptions, routeOptions, therDupOptions
...
therDupOptions = "<option value="""">-- Select --</option>" & _
                 "<option>Duplicate Therapy</option>" & _
                 "<option>Potential</option>" & _
                 "<option>None</option>"

Dim drugListHTML, selectedDrugName
drugListHTML = ""
selectedDrugName = ""

If DrugID <> "" Then
    Dim sqlName, rsName, cmdName
    sqlName = "SELECT DrugName FROM Drug WHERE DrugID = ?"
    Set cmdName = Server.CreateObject("ADODB.Command")
    cmdName.ActiveConnection = conn
    cmdName.CommandText = sqlName
    cmdName.Parameters.Append cmdName.CreateParameter("@drugid", 200, 1, 50, DrugID)
    Set rsName = cmdName.Execute
    If Not rsName.EOF Then selectedDrugName = Server.HTMLEncode(rsName("DrugName"))
    rsName.Close
    Set rsName = Nothing
    Set cmdName = Nothing
End If
...
Dim rs, sql
sql = "SELECT DISTINCT Prescription.DrugID, Drug.DrugName " & _
      "FROM Prescription " & _
      "INNER JOIN Drug ON Drug.DrugID = Prescription.DrugID " & _
      "WHERE Prescription.VisitationID = ? " & _
      "ORDER BY Drug.DrugName"

Dim cmdList
Set cmdList = Server.CreateObject("ADODB.Command")
cmdList.ActiveConnection = conn
cmdList.CommandText = sql
cmdList.Parameters.Append cmdList.CreateParameter("@visid", 200, 1, 50, VisitationID)
Set rs = cmdList.Execute

Do While Not rs.EOF
    drugListHTML = drugListHTML & "<option value=""" & Server.HTMLEncode(rs("DrugName")) & """ data-id=""" & Server.HTMLEncode(rs("DrugID")) & """>" & Server.HTMLEncode(rs("DrugName")) & "</option>"
    rs.MoveNext
Loop
rs.Close
Set rs = Nothing
...
Response.Write "<style>datalist { display: none; }</style>"
Response.Write "</head><body class='bg-light p-4'>"
