...
str = str & "<td width=""10""></td>"
str = str & "<td name=""tdInputInpInsuranceTypeID"" id=""tdInputInpInsuranceTypeID"">" & GetDoctorName(GetRecordField("LabRequestID")) & "</td>"
str = str & "<td width=""10""></td>"
...
str = str & "<td width=""10""></td>"
str = str & "<td name=""tdInputInpInsuranceTypeID"" id=""tdInputInpInsuranceTypeID"">&nbsp;" & GetClinicalDiagnosis(GetRecordField("LabRequestID")) & "</td>"
str = str & "<td width=""10""></td>"
End If
...
Function GetClinicalDiagnosis(vst)
  Dim ot, rst, sql
  sql = "select clinicaldiagnosis from labrequest where labrequestid='" & vst & "'"
  Set rst = server.CreateObject("ADODB.Recordset")
  With rst
  .open qryPro.FltQry(sql), conn, 3, 4
  If .RecordCount > 0 Then
  ot = .fields("clinicaldiagnosis")
  End If
  .Close
  GetClinicalDiagnosis = ot
  End With
End Function
...
Function GetDoctorName(vst)
  Dim ot, rst, sql
  sql = "select DoctorName from labrequest where labrequestid='" & vst & "'"
  Set rst = server.CreateObject("ADODB.Recordset")
  With rst
  .open qryPro.FltQry(sql), conn, 3, 4
  If .RecordCount > 0 Then
  ot = .fields("DoctorName")
  End If
  .Close
  GetDoctorName = ot
  End With
