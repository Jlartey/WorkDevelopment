...
If Len(GetComboName("Ward", jSchd)) > 0 Then
  wrd = jSchd 
End If
...
If UCase(sltTyp) = "NO" Then 'No Full Screen
    ...
    If Len(GetComboName("Ward", jSchd)) > 0 Then
      dyHt = dyHt & "<option value=""" & jSchd & """ selected>" & GetComboName("Ward", jSchd) & "</option>"
    Else
  ...
  Else ' Full Screen
    ...
    mth = ""
    With rst
      .open "select distinct WardID from Admission " & GetWardWhCls() & " and AdmissionStatusID='A001' and AdmissionStatusID='A001' and Admission.TransProcessValID in ('AdmissionPro-T001', 'AdmissionPro-T002', 'AdmissionPro-T014') ", conn, 3, 4
      If .recordCount > 0 Then
        .MoveFirst
        Do While Not .EOF
          wrd2 = Trim(.fields("WardID"))
...
Function GetWardWhCls()
  ...
  If Len(GetComboName("Ward", jb)) > 0 Then 'Ward Nurse
    ot = " where WardID='" & jb & "'"
  ElseIf UCase(Left(jb, 2)) = "W0" Then 'Ward Nurse
  ...
  ElseIf (UCase(jb) = "S19A") Or (UCase(Left(jb, 3)) = "M03") Or (UCase(Left(jb, 3)) = "M06") Or (UCase(jb) = "M0603") Or (UCase(jb) = "M0210") Or (UCase(jb) = "M0211") Or (UCase(jb) = "M0209") Then 'Doctor/Pharmacist/Theatre/Anaethesia/Public Health
    ot = " where WardID<>''"
    If Len(wrd) > 0 Then
      ot = " where WardID='" & wrd & "'"
    End If
    If UCase(jb) = "M0327" Then
        ot = " where WardID IN ('W009', 'W010' ) "
    End If
  ElseIf UCase(Left(jb, 3)) = "M02" Then 'OPD Nurse
    ...
    If UCase(jb) = "M0227" Then
        ot = " where WardID IN ('W009', 'W010' ) "
    End If
  ElseIf (UCase(jb) = "BILLINGHEAD") Or (UCase(jb) = "CLAIMMANAGER") Or (UCase(jb) = "DPT014") Or (UCase(jb) = "REGVISITWARD") Or (UCase(jb) = "REGVISITCON") Then 'BILLING
    ot = " where WardID<>''"
    If Len(wrd) > 0 Then
      ot = " where WardID='" & wrd & "'"
    End If
  ElseIf (UCase(jb) = "MANAGEMENT") Then 'Management
    ...
    If Len(wrd) > 0 Then
      ot = " where WardID='" & wrd & "'"
    End If
  ElseIf (UCase(jb) = "S93") Or (UCase(jb) = "S13") Or (UCase(jb) = "LabCashier") Or (UCase(jb) = "DPT005") Then 'Labs: MnC, Main
    ot = " where BranchID='" & brnch & "' "
  ElseIf (UCase(jb) = "S95") Or (UCase(jb) = "S22") Or (UCase(jb) = "S22A") Then  'Pharmacies: MnC, Main
    ot = " where BranchID='" & brnch & "' "
  ElseIf (UCase(jb) = "M13") Then 'Billing
    ot = " where BranchID='" & brnch & "' "
  ElseIf (UCase(jb) = "DPT011") Or (UCase(jb) = "S19") Then 'radiologist
    ot = " where BranchID='" & brnch & "'"
  ElseIf (UCase(jb) = "DPT010") Or (UCase(jb) = "DPT022") Then 'ward
    ot = " where BranchID='" & brnch & "'"
  Else
