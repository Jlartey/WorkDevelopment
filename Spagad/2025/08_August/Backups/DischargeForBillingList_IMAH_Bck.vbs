'<<--BEGIN_CODE_SEGMENT_PRINTHEADER-->>
    ...
    'M010 - T006
    'M011 - T007
    Dim str, lnkCnt, wrd, jb, lstWhCls, lstWhCls2, dispTyp, wdNm, iFUrl
    lnkCnt = 0
    jb = jSchd
...
Function isWard(jb)
    ...
    End Sub
    '
    'Function PatientBillPaid(vst)
    '  Dim sql, rstPrn1, rstPrn2, cat, catNm, tot, hdr, pos, gTot, gPaid, gUsed, pat, gWv
    '  Dim withReg, wd, insTyp, gIns, insWv, otTot, rTyp, wvPer, ot
    ...
    '
    '  sql = "select InsuranceTypeID,ReceiptTypeID,PatientID,VisitInfo1,VisitCost from Visitation"
    '  sql = sql & " where Visitationid='" & Trim(vst) & "'"
    '
    '  With rstPrn1
    '    .Open sql, conn, 3, 4
    '    If .RecordCount > 0 Then
    ...
    '      End If
    '    End If
    '    'gTot = gTot + AddRegister(pat, dy)
    '    gTot = gTot + .Fields("Visitcost")
    '
    '    gTot = gTot + AddAdmission(vst)
    '
    '    gTot = gTot + AddDrug(vst)
    '    gTot = gTot + AddDrug2(vst)
    '
    '    gTot = gTot + AddNonDrug(vst)
    '
    '    gTot = gTot + AddLab(vst)
    '    gTot = gTot + AddLab2(vst)
    '
    '    gTot = gTot + AddTreat(vst, gDys)
    '
    '    'Add Waivers
    '    gWv = AddWaiver(vst)
    '    gTot = gTot - gWv
    ...
    '    'Payments
    '    gPaid = AddPayments(pat, recNo, vst)
    '    'UsedPayments
    '    gUsed = AddUsedPayments(pat, recNo, vst)
    '
    '    'Grand Total
    '    otTot = gTot - gPaid + gUsed - gWv
    '    'Refund Required
    '    If (gTot - gWv) < 0 Then
    '      otTot = gUsed - gPaid
    '    End If
    ...
    '
    ''AddPayments
    'Function AddPayments(pat, recNo, vst)
    '  Dim rst, sql, ot, cnt, hdr, dsc, pd, cn, bal, dt, sDt, eDt, dDt, cn2
    '  Dim arr, ul, num, whcls, r, rCnt, sqlOk, sql2, regRec, paid
    '  Set rst = CreateObject("ADODB.Recordset")
    '
    '  paid = 0
    '  'Receipt No
    ...
    '
    ''AddUsedPayments
    'Function AddUsedPayments(pat, recNo, vst)
    '  Dim rst, rst2, sql, ot, cnt, hdr, dsc, pd, cn, bal, dt, sDt, eDt, dDt
    '  Dim cnt2, cn2, rec, usd, uCnt, sql2, regRec, used
    ...
    '  Set rst = Nothing
    '  Set rst2 = Nothing
    '   AddUsedPayments = used
    'End Function
    '
    ''AddAdmission
    'Function AddAdmission(vst)
    '  Dim rst, sql, ot, cnt, hdr, adm, chg, dys, aDt, dDt, gADt, gDDt, dyCnt, gDyCnt, recCnt, aTot
    '  Set rst = CreateObject("ADODB.Recordset")
    ...
    '  Dim rst, sql, ot, cnt, hdr, adm, unt, qty, tot, rQty, drg, fQty, rNo, dTot
    '  Set rst = CreateObject("ADODB.Recordset")
    ...
    'End Function
    ''AddDrug2
    'Function AddDrug2(vst)
    '  Dim rst, sql, ot, cnt, hdr, adm, unt, qty, tot, rQty, drg, fQty, rNo, dTot
    '  Set rst = CreateObject("ADODB.Recordset")
    '  sql = "select drugid,sum(qty) as qty,avg(unitcost) as unt,sum(finalamt) as tot from drugsaleitems2 where visitationid='" & vst & "' group by drugid"
    '  dTot = 0
    '  With rst
    '  .Open sql, conn, 3, 4
    '  If .RecordCount > 0 Then
    ...
    'Dim rst, sql, ot, cnt, hdr, adm, unt, qty, tot, nTot
    'Set rst = CreateObject("ADODB.Recordset")
    ...
    '  Dim rst, sql, ot, cnt, hdr, adm, unt, qty, tot, wV
    '
    '  wV = 0
    '  Set rst = CreateObject("ADODB.Recordset")
    ...
    ''AddLab
    'Function AddLab(vst)
    'Dim rst, sql, ot, cnt, hdr, adm, unt, qty, tot, rNo, lbTot
    'Set rst = CreateObject("ADODB.Recordset")
    ...
    'End Function
    ''AddLab
    'Function AddLab2(vst)
    'Dim rst, sql, ot, cnt, hdr, adm, unt, qty, tot, rNo, lbTot
    'Set rst = CreateObject("ADODB.Recordset")
    'sql = "select labtestid,sum(qty) as qty,avg(unitcost) as unt,sum(finalamt) as tot from investigation2 where visitationid='" & vst & "'"
    ''sql = sql & " and testcategoryid<>'T006' and testcategoryid<>'T007' and testcategoryid<>'T008' group by labtestid"
    'sql = sql & " group by labtestid"
    'lbTot = 0
    'With rst
    ...
    '
    ''AddXRay
    'Function AddXray(vst)
    'Dim rst, sql, ot, cnt, hdr, adm, unt, qty, tot, xTot
    'Set rst = CreateObject("ADODB.Recordset")
    'sql = "select labtestid,sum(qty) as qty,avg(unitcost) as unt,sum(finalamt) as tot from investigation where visitationid='" & vst & "'"
    ...
    'End Function
    ''AddRegister
    'Function AddRegister(pat, dy)
    'Dim rst, sql, ot, cnt, hdr, chg, pTot
    'Set rst = CreateObject("ADODB.Recordset")
    ...
    'End Function
    ''AddTreat
    'Function AddTreat(vst, gDys)
    'Dim rst, sql, ot, cnt, hdr, adm, unt, qty, tot, rNo, dailyChr, trid, trNm, trTot
    'Set rst = CreateObject("ADODB.Recordset")
    ...
    'trTot = trTot + tot
    '
    'dailyChr = .Fields("dlyChr")
    'If IsNumeric(dailyChr) And IsNumeric(gDys) Then
    '  If CDbl(dailyChr) > 0 Then
    '    tot = CDbl(dailyChr) * gDys
    '    trTot = trTot + tot
    ' End If
    'End If
    '.movenext
