...
Function GetWardWhCls()
  ...
  ElseIf (UCase(jb) = "BILLINGHEAD") Or (UCase(jb) = "CLAIMMANAGER") Or (UCase(jb) = "REGVISITWARD") Or (UCase(jb) = "REGVISITCON") Then 'BILLING
    ...
    If Len(wrd) > 0 Then
      ot = " where WardID='" & wrd & "'"
    End If
  ElseIf (UCase(jb) = "MANAGEMENT") Or (UCase(jb) = UCase("chiefexecutiveofficer")) Or (UCase(jb) = "MANAGEMENT") Then 'Management
    ot = " where WardID<>''"
    If Len(wrd) > 0 Then
      ot = " where WardID='" & wrd & "'"
    End If
  ElseIf (UCase(jb) = "S93") Or (UCase(jb) = "S13") Or (UCase(jb) = "LabCashier") Then 'Labs: MnC, Main
    ot = " where BranchID='" & brnch & "' "
  ElseIf (UCase(jb) = "DPT005") Then  'Labhead '13TH MARCH 2023 Monica
    ot = " where BranchID='" & brnch & "' "
