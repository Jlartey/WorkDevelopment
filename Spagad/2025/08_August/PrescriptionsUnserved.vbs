...
Dim periodStart, periodEnd, specialistType

datePeriod = Trim(Request.QueryString("PrintFilter"))
specialistType = Trim(Request.QueryString("PrintFilter1"))
...
Sub dispUndispensedDrugs()
    ...
    If specialistType <> "" Then
      sql = sql & "AND Visitation.SpecialistTypeID = '" & specialistType & "'  
    End If
    ...
    With rst
        ...
        If .recordCount > 0 Then
            response.write "<h3 style='font-family: Arial;'>Showing " & .recordCount & " rows</h3>
...
Function FormatPrescriptions(drugString)
    Dim drugArray, htmlOutput
    
    ' Check if drugString is null or empty
    If IsNull(drugString) Or Trim(drugString) = "" Then
        htmlOutput = "<p>No prescriptions served</p>"
    Else
        ...
        For i = 0 To UBound(drugArray)
            ...
            If Len(drug) > 0 Then
                htmlOutput = htmlOutput & "    <li>" & Server.HTMLEncode(drug) & "</li>" & vbCrLf
            End If
        Next
        htmlOutput = htmlOutput & "</ol>"
        ' If no valid drugs were found in the array, return "No prescriptions served"
        If htmlOutput = "<ol>" & vbCrLf & "</ol>" Then
            htmlOutput = "<p>No prescriptions served</p>"
        End If
    End If
...
Function DrugsSold(VisitationID)
    ...
    ' Check if VisitationID is null or empty
    If IsNull(VisitationID) Or Trim(VisitationID) = "" Then
        DrugsSold = "<p>No drugs sold</p>"
        Exit Function
    End If
    ...
    ' Check if query returned no records or DrugName is null/empty
    If rst.EOF Or rst.recordCount = 0 Then
        htmlOutput = "<p>No drugs sold</p>"
    Else
        htmlOutput = "<ol>" & vbCrLf
        While Not rst.EOF
            Dim drugName
        ...
        Wend
        htmlOutput = htmlOutput & "</ol>"
        ' If no valid drug names were found, return "No drugs sold"
        If htmlOutput = "<ol>" & vbCrLf & "</ol>" Then
            htmlOutput = "<p>No drugs sold</p>"
        End If
    End If
