...
Function getBatchDropDown(drugID)
    Dim sqlDropdown, dropdownOptions

    sqlDropdown = "SELECT DISTINCT PurchaseOrderInfo1 FROM IncomingDrugItems WHERE DrugID = '" & drugID & "' "
    
    'response.write sqlDropdown
    Set rstDropdown = CreateObject("ADODB.Recordset")
    rstDropdown.open qryPro.FltQry(sqlDropdown), conn, 3, 4
    ...
    With rstDropdown
      If .RecordCount > 0 Then
        .movefirst
        Do While Not .EOF
          dropdownOptions = dropdownOptions & "<option value='" & .fields("PurchaseOrderInfo1") & "'>" & .fields("PurchaseOrderInfo1") & "</option>"
          .MoveNext
        Loop
      End If
    End With
    ...
    getBatchDropDown = dropdownOptions
End Function

'response.write "Dropdown Options are " & dropdownOptions
...
'      sql = sql & " order by drug.drugName"
     ...
     'My addition
    sql = "SELECT DISTINCT drug.drugname, drug.RetailUnitCost, drugstocklevel.drugid "
...
'          response.write "</tr>"
        ...
        response.write "<td valign=""top"" align=""Center"" width=""140""><b>BATCH NO.</b></td>"
        response.write "<td valign=""top"" align=""Center"" width=""100""><b>EXPIRY DATE</b></td>"
        response.write "</tr>"
          ...
          Do While Not .EOF And recCnt < CInt(recPerPg)
            ...
            response.write "<td align=""center""><select class=""batchNo"" name=""batchNo"">" & getBatchDropDown(drg) & "</select></td>"
            response.write "<td align=""right""></td>"
...
'          response.write "</tr>"
  ...
  Set rst0 = Nothing
    ...
    response.write "   var batchSelects = document.getElementsByName('batchNo');"
    response.write "   for (var i = 0; i < batchSelects.length; i++) {"
    response.write "       batchSelects[i].addEventListener('change', function() {"
    response.write "           var selectElement = this;" ' Store the reference to the select element
    response.write "           var row = selectElement.parentNode.parentNode;" ' Traverse up to the parent <tr> element
    response.write "           var drugID = row.cells[1].innerText;" ' Get drug ID from the second <td> in the <tr>
    response.write "           var batchNum = selectElement.value;" ' Get the selected batch number"
    response.write "           var xhr = new XMLHttpRequest();"
    response.write "           var url = 'wpgxmlhttp.asp?procedurename=dispExpiryDate&drugID=' + encodeURIComponent(drugID) + '&batchNum=' + encodeURIComponent(batchNum);"
    ...
    response.write "                   var expiryDateCell = row.cells[9];" ' Get the 10th <td> in the <tr> for the expiry date
    response.write "                   expiryDateCell.innerHTML = xhr.responseText;" ' Populate the expiry date field"
    response.write "               }"
    response.write "           };"
