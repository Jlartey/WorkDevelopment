...
Function getBatchDropDown(drugID)
    ...
    sqlDropdown = "SELECT DISTINCT InventoryRefNo FROM IncomingDrugItems WHERE DrugID = '" & drugID & "' "
    ...
    With rstDropdown
      If .RecordCount > 0 Then
        .MoveFirst
        Do While Not .EOF
          dropdownOptions = dropdownOptions & "<option value='" & .fields("InventoryRefNo") & "'>" & .fields("InventoryRefNo") & "</option>"
          .MoveNext
        Loop
      End If
    End With
...
'          response.write "</tr>"
          ...
          Do While Not .EOF And recCnt < CInt(recPerPg)
            ...
            response.write "<td align=""center""><select id=""batchNo"" name=""batchNo"">" & getBatchDropDown(drg) & "</select></td>"
