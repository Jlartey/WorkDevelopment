...
Sub DatePicker()
    ...
    response.write "                <option value='SF32'>SF32</option>"
...
Function getSF32_Score(EMRRequestID)
    Dim sql, rst, userScore, totalScore, getSF_32ScoreDivided
    Set rst = CreateObject("ADODB.RecordSet")

   'physical function  
    sql = sql & "select sum(score) as userScore, sum(total) as totalScore from ( "
    sql = sql & "select SUM((50 * varpos) - 50) as score, count(varpos) as total fom EMRResults e, emrvar3b e2 "
    sql = sql & "where cast(e.Column3 as varchar) = e2.EMRVar3BID "
    sql = sql & "and EMRRequestID =  'E100085851' "
    sql = sql & "AND e.EmrComponentID IN ('RES017003', 'RES017004', 'RES017005', 'RES017006', 'RES017007') "
    sql = sql & "UNION ALL "

    sql = sql & "select sum((50 * varpos) - 50) as score, count(varpos) as total from EMRResults e, emrvar3b e2 "
    sql = sql & "where cast(e.Column6 as varchar) = e2.EMRVar3BID and EMRRequestID =  'E100085851'"
    sql = sql & "AND e.EmrComponentID IN ('RES017003', 'RES017004', 'RES017005', 'RES017006', 'RES017007') "
    sql = sql & "UNION ALL"
       ...
       .open qryPro.FltQry(sql), conn, 3, 4
        If .RecordCount > 0 Then
            ...
            ' Multiply totalScore by 4 and calculate the division
               ...
               getSF_32Score = "" &  getSF_32ScoreDivided & ""
            End If
        End If
    .Close
    End With
...
Function getScore(emrreqid,emrdataid)
    ...
    Select Case emrdataid
        ...
        Case "RES007"
            tscore = getEMRResult(emrreqid,emrdataid,"res007027	", "Column2")
        Case "RES013"
            tscore = getEMRResult(emrreqid,emrdataid,"RES013012", "Column2")
    End Select
    getScore = tscore
End Function
