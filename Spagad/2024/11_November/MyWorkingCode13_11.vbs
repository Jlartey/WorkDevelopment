...
'    response.write "        }"
    ...
    response.write "        params.set('PrintLayoutName', 'DispResearchRpt');"
...
'E100169161, E100169159, E100110234

Sub displayEOSQ()
  Dim url, vst, periodStart, periodEnd, cnt
  cnt = 0
  Set rst = CreateObject("ADODB.RecordSet")
      ...
      .open qryPro.FltQry(sql), conn, 3, 4
          If .RecordCount > 0 Then
              ...
              Do While Not .EOF
                ...
                response.write "       <td>" & PatientID & "</td>"
                response.write "       <td>" & GetComboName("Patient", PatientID) & "</td>"
                response.write "       <td>" & getSF32_Score(EMRRequestID) & "</td>"
                response.write "     </tr>"
...
Function getSF32_Score(EMRRequestID)
    Dim sql, rst, userScore, totalScore, getSF32_ScoreDivided
    Set rst = CreateObject("ADODB.RecordSet")
       ...
       .open qryPro.FltQry(sql), conn, 3, 4
        If .RecordCount > 0 Then
            ...
            Else
               ...
               getSF32_ScoreDivided = FormatNumber(userScore / totalScore, 2)
               getSF32_Score = "" & getSF32_ScoreDivided & ""
            End If
        End If
    .Close
    End With
