...
Sub ShowSearchResult()
    ...
    End If
    If Len(insno) > 0 Then
        If Len(pat) > 0 Or Len(spn) > 0 Then
            sql = sql & " and "
        Else
            sql = sql & " or "
        End If
        sql = sql & " Patient.PatientID in (select InsuredPatient.PatientID from InsuredPatient where InsuredPatient.InsuranceNo='" & insno & "')"
    End If
    If Len(phNo) > 0 Then
        If Len(pat) > 0 Or Len(insno) > 0 Or Len(spn) > 0 Then
            sql = sql & " and "
        Else
            sql = sql & " or "
        End If
