...
Sub ShowSearchResult()
    ...
    ' sql = sql & " inner join InsuredPatient on InsuredPatient.PatientID = Patient.PatientID"
    ' sql = sql & " inner join Sponsor on Sponsor.SponsorId = InsuredPatient.SponsorID"
    ' sql = sql & " where 1=0 "
    
    sql = "SELECT DISTINCT TOP 100 Patient.PatientID, Patient.PatientName, Patient.GenderID, Patient.BirthDate, Patient.ResidencePhone, "
    sql = sql & "CAST(Patient.ResidenceAddress AS VARCHAR(MAX)) AS ResidenceAddress, Patient.Occupation "
    sql = sql & "FROM Patient "
    sql = sql & "INNER JOIN InsuredPatient ON InsuredPatient.PatientID = Patient.PatientID "
    sql = sql & "INNER JOIN Sponsor ON Sponsor.SponsorId = InsuredPatient.SponsorID "
    sql = sql & "WHERE 1=0 "
    ...
    If Len(pat) > 0 Then
        ...
        For Each tmp In Split(pat, " ")
            If tmp2 <> "" Then
                tmp2 = tmp2 & " and "
            End If
            tmp2 = tmp2 & " (Patient.PatientID like '%" & Trim(tmp) & "%' "
            tmp2 = tmp2 & " or Patient.PatientName like '%" & Trim(tmp) & "%' "
            tmp2 = tmp2 & " or Patient.ServiceNo like '%" & Trim(tmp) & "%')"
        Next
        If tmp2 <> "" Then
            sql = sql & " or (" & tmp2 & ") "
        End If
    ...
    End If
    If Len(phNo) > 0 Then
        If Len(pat) > 0 Or Len(insNo) Or Len(spn) > 0 Then
            sql = sql & " and "
        Else
            sql = sql & " or "
        End If
    ...
    End If
    If Len(spn) > 0 Then
        If Len(pat) > 0 Or Len(insNo) > 0 Or Len(phNo) > 0 Then
            sql = sql & " and "
        Else
            sql = sql & " or "
        End If
        sql = sql & " Sponsor.SponsorName like '%" & spn & "%' "
    End If
    ...
    response.write "<td align=""center""><b>Sponsors</b></td>"
...
End Sub
Sub DisplayUserSponsors(pID)
 Dim rst, sql, spid
 Set rst = CreateObject("ADODB.RecordSet")
 sql = "SELECT sponsorID FROM InsuredPatient WHERE patientid='" & pID & "'"
 With rst
  ...
  If .RecordCount > 0 Then
    ...
    Do While Not .EOF
        ...
        If spid <> "SELF" Then
            response.write "<small style='display:block';color:#444488;>" & GetComboName("Sponsor", spid) & "</small>"
        End If
        .MoveNext
    Loop
  End If
...
Sub ShowSearchPage()
    ...
    'My additions @JL
    response.write "                                                 <tr class=""pnlOff""> "
    response.write "                                                     <td name=""tdLabelInpFltInputText||8"""
    response.write "                                                         id=""tdLabelInpFltInputText||8"" align=""right"">Sponsor</td>"
    response.write "                                                     <td name=""tdInputInpFltInputText||8"""
    response.write "                                                         id=""tdInputInpFltInputText||8"""
    response.write "                                                         style=""font-size: 9pt; font-family: Arial""><input"
    response.write "                                                             type=""text"" name=""PrintFilter3"""
    response.write "                                                             id=""PrintFilter3"" size=""30"" value=""" & request("PrintFilter3") & """></td>"
    response.write "                                                 </tr> "
