...
Sub dispPatientVisitations01()
    ...
    Dim pageSize, pageNum, totalPages
    pageSize = 10 ' Number of records per page
    pageNum = 1   ' Default page number, can be adjusted based on request
    ...
    If pageNum > 1 Then
        Response.Write "<a href='?page=1&Dateperiod=" & datePeriod & "&WardID=" & selectedWardIDs & "'>First</a>"
        Response.Write "<a href='?page=" & (pageNum - 1) & "&Dateperiod=" & datePeriod & "&WardID=" & selectedWardIDs & "'>Previous</a>"
    End If

    Response.Write "Page " & pageNum & " of " & totalPages

    If pageNum < totalPages Then
        Response.Write "<a href='?page=" & (pageNum + 1) & "&Dateperiod=" & datePeriod & "&WardID=" & selectedWardIDs & "'>Next</a>"
        Response.Write "<a href='?page=" & totalPages & "&Dateperiod=" & datePeriod & "&WardID=" & selectedWardIDs & "'>Last</a>"
    End If
...
Sub dispPatientVisitations02()
    ...
    Response.Write "<div class='pagination'>"
    If pageNum > 1 Then
        Response.Write "<a href='?page=1&Dateperiod=" & datePeriod & "&WardID=" & selectedWardIDs & "&pageSize=" & pageSize & "'>First</a>"
        Response.Write "<a href='?page=" & (pageNum - 1) & "&Dateperiod=" & datePeriod & "&WardID=" & selectedWardIDs & "&pageSize=" & pageSize & "'>Previous</a>"
    End If

    Response.Write "Page " & pageNum & " of " & totalPages

    If pageNum < totalPages Then
        Response.Write "<a href='?page=" & (pageNum + 1) & "&Dateperiod=" & datePeriod & "&WardID=" & selectedWardIDs & "&pageSize=" & pageSize & "'>Next</a>"
        Response.Write "<a href='?page=" & totalPages & "&Dateperiod=" & datePeriod & "&WardID=" & selectedWardIDs & "&pageSize=" & pageSize & "'>Last</a>"
    End If
...
Sub dispPatientVisitations03()
    ...
    Response.Write "  var url = '?page=' + nextPage + '&Dateperiod=" & datePeriod & "&WardID=" & selectedWardIDs & "&pageSize=" & pageSize & "';"
...
End Sub
Sub dispPatientVisitationsAlternative()
    Dim conn, sql, periodStart, periodEnd, datePeriod, selectedWardIDs, idsArr, formattedIDs, id, dateArr
    Dim rstDropdown, rstMain
    Dim dropdownOptions, optionHTML, pageSize, pageNum, totalPages
    ...
    ' Execute dropdown options query
    Set rstDropdown = conn.Execute(sql)

    ' Populate dropdown options
    dropdownOptions = ""
    If Not rstDropdown.EOF Then
        Do Until rstDropdown.EOF
            optionHTML = "<option value='" & rstDropdown("WardID") & "'>" & rstDropdown("Wardname") & "</option>"
            dropdownOptions = dropdownOptions & optionHTML
            rstDropdown.MoveNext
        Loop
    End If
    ...
    ' Construct base SQL query for main data
    sql = "SELECT "
    sql = sql & "Patient.PatientID, "
    sql = sql & "Patient.Patientname, "
    sql = sql & "VisitType.VisitTypeName, "
    ...
    ' Order by AdmissionDate in descending order
    sql = sql & "ORDER BY CONVERT(date, Admission.AdmissionDate) DESC"

    ' Execute main data query
    Set rstMain = conn.Execute(sql)

    ' Calculate total pages
    totalPages = Int((rstMain.RecordCount + pageSize - 1) / pageSize)

    ' Output dropdown and date selection form
    Response.Write "<div>"
    Response.Write "    <label for='ward' class='font-style'>Select Ward:</label><br>"
    Response.Write "    <select id='ward' name='ward' multiple class='mult-select-tag'>"
    Response.Write dropdownOptions
    ...
    Response.Write "        </div> "
    Response.Write "        <div> "
    Response.Write "            <button type='button' onclick='showNextPage()'>Show Next Page</button>"
    Response.Write "        </div>    "
    Response.Write "    </div> "
    Response.Write "</form>"

    ' Display data for the current page
    DisplayPageData pageNum, pageSize, rstMain
    ...
    ' Load more button using JavaScript
    Response.Write "<script>"
    Response.Write "function showNextPage() {"
    Response.Write "  var nextPage = " & pageNum + 1 & ";"
    Response.Write "  var url = '?page=' + nextPage + '&Dateperiod=" & datePeriod & "&WardID=" & selectedWardIDs & "&pageSize=" & pageSize & "';"
    Response.Write "  window.location.href = url;"
    Response.Write "}"
    ...
    Response.Write "<div style='text-align: center; margin-top: 20px;'>"
    If pageNum < totalPages Then
        Response.Write "<button onclick='showNextPage()'>Show Next Page</button>"
    End If
    Response.Write "</div>"
End Sub

Sub DisplayPageData(pageNum, pageSize, rstMain)
    Dim count
    count = 0
...
Sub dispPatientVisitationsAlternative01()
    Dim sql, periodStart, periodEnd, datePeriod, count, selectedWardIDs, idsArr, formattedIDs, id, dateArr
    Dim rstDropdown, rstMain
    Dim dropdownOptions, optionHTML
