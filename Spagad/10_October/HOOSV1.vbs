...
AddCSSS
Dim sql, sql1, rst, rst1, cnt, vst, sponsor, gen, spclst
...
Function getScore(EMRRequestID)
  Dim sql, rst, userScore, totalScore, getScoreDivided
  Set rst = CreateObject("ADODB.RecordSet")
  ...
  With rst
    ...
    If .RecordCount > 0 Then
      ...
      Else
        ...
        getScoreDivided = FormatNumber((userScore / (totalScore * 4)) * 100)
        getScore = "" & getScoreDivided & ""
      End If
    End If
    .Close
  End With
...
' Function getScore01(EMRRequestID)
'   Dim sql, rst
'   Set rst = CreateObject("ADODB.RecordSet")
